...
public class CreateFileManagerCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateFileManagerCommand command = new CreateFileManagerCommand(
				FileManagerId.create(UUID.randomUUID().toString()),
				FileManagerName.create(UUID.randomUUID().toString()),
				FileManagerDetails.create(UUID.randomUUID().toString()),
				FileManagerObjectId.create(UUID.randomUUID().toString()),
				FileManagerObjectName.create(UUID.randomUUID().toString()),
				FileManagerOriginalName.create(UUID.randomUUID().toString()),
				FileManagerMimeType.create(UUID.randomUUID().toString()),
				FileManagerSize.create(47166L),
				FileManagerPath.create(UUID.randomUUID().toString()),
				FileManagerUri.create(UUID.randomUUID().toString()),
				FileManagerIsPublic.create(false),
				FileManagerCreatedBy.create(UUID.randomUUID().toString()),
				FileManagerTenant.create(UUID.randomUUID().toString())
		);
		commandGateway.send(command);

		ArgumentCaptor<CreateFileManagerCommand>
				commandCaptor =
				ArgumentCaptor.forClass(CreateFileManagerCommand.class);
