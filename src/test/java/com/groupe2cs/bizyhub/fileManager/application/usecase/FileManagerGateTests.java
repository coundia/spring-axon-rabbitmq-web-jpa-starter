...
class FileManagerGateTests {

	@Mock
	private FileManagerRepository repository;
	@InjectMocks
	private FileManagerGate gate;

	private FileManager sampleFileManager;
	...
	void setUp() {
		...
		sampleFileManager = FileManager.builder()
				.id("tx-123")
				.createdBy(user1)
				.tenant(tenantA)
				.build();
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleFileManager.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleFileManager.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleFileManager.getId()))
					.thenReturn(Optional.of(sampleFileManager));
			assertThat(gate.canRead(auth, sampleFileManager.getId())).isEqualTo(expected);
			verify(repository).findById(sampleFileManager.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleFileManager.getId()))
				.thenReturn(Optional.of(sampleFileManager));
		assertThat(gate.canEdit(ownerAuth, sampleFileManager.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleFileManager.getId())).isTrue();
	}
}
