...
void it_should_create_setting_with_valid_values() {
	...
	SettingDescriptions descriptions = SettingDescriptions.create(UUID.randomUUID().toString());
	...
	SettingAggregate aggregateSettingAggregate = new SettingAggregate(id, name, reference, updatedAt, stringValue, descriptions, isActive, createdBy, tenant);
	...
	assertThat(aggregateSettingAggregate.getDescriptions()).isEqualTo(descriptions);
	...
	assertThat(aggregateSettingAggregate.getTenant()).isEqualTo(tenant);
		...
		SettingDescriptionsNotValid error = assertThrows(SettingDescriptionsNotValid.class, () -> SettingDescriptions.create(""));
