...
class SettingGateTests {

	@Mock
	private SettingRepository repository;
	@InjectMocks
	private SettingGate gate;

	private Setting sampleSetting;
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleSetting.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleSetting.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleSetting.getId()))
					.thenReturn(Optional.of(sampleSetting));
			assertThat(gate.canRead(auth, sampleSetting.getId())).isEqualTo(expected);
			verify(repository).findById(sampleSetting.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleSetting.getId()))
				.thenReturn(Optional.of(sampleSetting));
		assertThat(gate.canEdit(ownerAuth, sampleSetting.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleSetting.getId())).isTrue();
	}
}
