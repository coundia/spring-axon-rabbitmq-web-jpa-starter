...
import com.groupe2cs.bizyhub.settings.infrastructure.entity.Settings;
...
sampleSettings = Settings.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleSettings.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleSettings.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleSettings.getId()))
	.thenReturn(Optional.of(sampleSettings));
	assertThat(gate.canRead(auth, sampleSettings.getId())).isEqualTo(expected);
	verify(repository).findById(sampleSettings.getId());
	}
	}
	...
	when(repository.findById(sampleSettings.getId()))
	.thenReturn(Optional.of(sampleSettings));
	assertThat(gate.canEdit(ownerAuth, sampleSettings.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleSettings.getId())).isTrue();
	}
	}
