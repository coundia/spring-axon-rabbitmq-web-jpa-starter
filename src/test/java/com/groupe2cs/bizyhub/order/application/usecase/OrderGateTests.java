...
class OrderGateTests {

	@Mock
	private OrderRepository repository;
	@InjectMocks
	private OrderGate gate;
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleOrder.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleOrder.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleOrder.getId()))
					.thenReturn(Optional.of(sampleOrder));
			assertThat(gate.canRead(auth, sampleOrder.getId())).isEqualTo(expected);
			verify(repository).findById(sampleOrder.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleOrder.getId()))
				.thenReturn(Optional.of(sampleOrder));
		assertThat(gate.canEdit(ownerAuth, sampleOrder.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleOrder.getId())).isTrue();
	}
}
