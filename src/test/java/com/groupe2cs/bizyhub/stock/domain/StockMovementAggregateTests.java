...
public class StockMovementAggregateTests extends BaseUnitTests {
	...
	void it_should_create_stockMovement_with_valid_values() {
		...
		StockMovementAggregate
				aggregateStockMovementAggregate =
				new StockMovementAggregate(id,
						typeStockMovement,
						quantity,
						remoteId,
						localId,
	...
	void it_should_throw_when_typeStockMovement_is_invalid() {
		StockMovementTypeStockMovementNotValid
				error =
				assertThrows(StockMovementTypeStockMovementNotValid.class,
						() -> StockMovementTypeStockMovement.create(""));
		assertThat(error.getMessage()).isEqualTo("TypeStockMovement is invalid");
	}

	@Test
	void it_should_throw_when_quantity_is_invalid() {
		StockMovementQuantityNotValid
				error =
				assertThrows(StockMovementQuantityNotValid.class, () -> StockMovementQuantity.create(-1));
