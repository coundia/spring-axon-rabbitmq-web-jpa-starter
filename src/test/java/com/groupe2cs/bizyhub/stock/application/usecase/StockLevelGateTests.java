...
class StockLevelGateTests {

	@Mock
	private StockLevelRepository repository;
	@InjectMocks
	private StockLevelGate gate;

	private StockLevel sampleStockLevel;
	...
	void setUp() {
		...
		sampleStockLevel = StockLevel.builder()
				.id("tx-123")
				.createdBy(user1)
				.tenant(tenantA)
				.build();
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleStockLevel.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleStockLevel.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleStockLevel.getId()))
					.thenReturn(Optional.of(sampleStockLevel));
			assertThat(gate.canRead(auth, sampleStockLevel.getId())).isEqualTo(expected);
			verify(repository).findById(sampleStockLevel.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleStockLevel.getId()))
				.thenReturn(Optional.of(sampleStockLevel));
		assertThat(gate.canEdit(ownerAuth, sampleStockLevel.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleStockLevel.getId())).isTrue();
	}
}
