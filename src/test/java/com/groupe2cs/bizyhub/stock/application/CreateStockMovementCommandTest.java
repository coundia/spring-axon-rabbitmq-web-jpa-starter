...
public class CreateStockMovementCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateStockMovementCommand command = new CreateStockMovementCommand(
				StockMovementId.create(UUID.randomUUID().toString()),
				StockMovementTypeStockMovement.create(UUID.randomUUID().toString()),
				StockMovementQuantity.create(95),
				StockMovementRemoteId.create(UUID.randomUUID().toString()),
				StockMovementLocalId.create(UUID.randomUUID().toString()),
				StockMovementAccount.create(UUID.randomUUID().toString()),
				StockMovementCompany.create(UUID.randomUUID().toString()),
				StockMovementSyncAt.create(java.time.Instant.now().plusSeconds(3600)),
				StockMovementProductVariant.create(UUID.randomUUID().toString()),
				StockMovementOrderLineId.create(UUID.randomUUID().toString()),
				StockMovementDiscriminator.create(UUID.randomUUID().toString()),
				StockMovementCreatedBy.create(UUID.randomUUID().toString()),
				StockMovementTenant.create(UUID.randomUUID().toString())
		);
		commandGateway.send(command);

		ArgumentCaptor<CreateStockMovementCommand>
				commandCaptor =
				ArgumentCaptor.forClass(CreateStockMovementCommand.class);
