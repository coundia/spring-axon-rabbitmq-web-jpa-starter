...
public class CreateStockLevelCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateStockLevelCommand command = new CreateStockLevelCommand(
				StockLevelId.create(UUID.randomUUID().toString()),
				StockLevelRemoteId.create(UUID.randomUUID().toString()),
				StockLevelLocalId.create(UUID.randomUUID().toString()),
				StockLevelStockOnHand.create(34),
				StockLevelStockAllocated.create(4),
				StockLevelAccount.create(UUID.randomUUID().toString()),
				StockLevelProductVariant.create(UUID.randomUUID().toString()),
				StockLevelSyncAt.create(java.time.Instant.now().plusSeconds(3600)),
				StockLevelCompany.create(UUID.randomUUID().toString()),
				StockLevelCreatedBy.create(UUID.randomUUID().toString()),
				StockLevelTenant.create(UUID.randomUUID().toString())
		);
