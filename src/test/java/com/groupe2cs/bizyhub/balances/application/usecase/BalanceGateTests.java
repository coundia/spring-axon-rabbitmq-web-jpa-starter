...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleBalance.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleBalance.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleBalance.getId()))
	.thenReturn(Optional.of(sampleBalance));
	assertThat(gate.canRead(auth, sampleBalance.getId())).isEqualTo(expected);
	verify(repository).findById(sampleBalance.getId());
	}
	}
	...
	when(repository.findById(sampleBalance.getId()))
	.thenReturn(Optional.of(sampleBalance));
	assertThat(gate.canEdit(ownerAuth, sampleBalance.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleBalance.getId())).isTrue();
	}
	}
