...
public class BalanceAggregateTests extends BaseUnitTests {
	...
	void it_should_create_balance_with_valid_values() {
		...
		BalanceCurrentBalance currentBalance = BalanceCurrentBalance.create(8552.11);
		BalancePreviousBalance previousBalance = BalancePreviousBalance.create(2092.01);
		BalanceLastUpdated lastUpdated = BalanceLastUpdated.create(java.time.Instant.now().plusSeconds(3600));
		BalanceIsDefault isDefault = BalanceIsDefault.create(false);
		BalanceOrdre ordre = BalanceOrdre.create(46);
		BalanceIsArchived isArchived = BalanceIsArchived.create(true);
		BalanceIsAllowDebit isAllowDebit = BalanceIsAllowDebit.create(true);
		BalanceIsAllowCredit isAllowCredit = BalanceIsAllowCredit.create(true);
		BalanceIsExcluTotal isExcluTotal = BalanceIsExcluTotal.create(false);
		BalanceActivateNotification activateNotification = BalanceActivateNotification.create(true);
		...
		BalanceAggregate
				aggregateBalanceAggregate =
				new BalanceAggregate(id,
						name,
						currency,
						currentBalance,
						previousBalance,
	...
	void it_should_throw_when_currentBalance_is_invalid() {
		BalanceCurrentBalanceNotValid
				error =
				assertThrows(BalanceCurrentBalanceNotValid.class, () -> BalanceCurrentBalance.create(null));
		assertThat(error.getMessage()).isEqualTo("CurrentBalance is invalid");
	}

	@Test
	void it_should_throw_when_previousBalance_is_invalid() {
		BalancePreviousBalanceNotValid
				error =
				assertThrows(BalancePreviousBalanceNotValid.class, () -> BalancePreviousBalance.create(null));
		assertThat(error.getMessage()).isEqualTo("PreviousBalance is invalid");
	}

	@Test
	void it_should_throw_when_lastUpdated_is_invalid() {
		BalanceLastUpdatedNotValid
				error =
				assertThrows(BalanceLastUpdatedNotValid.class, () -> BalanceLastUpdated.create(null));
		assertThat(error.getMessage()).isEqualTo("LastUpdated is invalid");
	}

	@Test
	void it_should_throw_when_isDefault_is_invalid() {
		BalanceIsDefaultNotValid
				error =
				assertThrows(BalanceIsDefaultNotValid.class, () -> BalanceIsDefault.create(null));
	...
	void it_should_throw_when_isArchived_is_invalid() {
		BalanceIsArchivedNotValid
				error =
				assertThrows(BalanceIsArchivedNotValid.class, () -> BalanceIsArchived.create(null));
		assertThat(error.getMessage()).isEqualTo("IsArchived is invalid");
	}

	@Test
	void it_should_throw_when_isAllowDebit_is_invalid() {
		BalanceIsAllowDebitNotValid
				error =
				assertThrows(BalanceIsAllowDebitNotValid.class, () -> BalanceIsAllowDebit.create(null));
		assertThat(error.getMessage()).isEqualTo("IsAllowDebit is invalid");
	}

	@Test
	void it_should_throw_when_isAllowCredit_is_invalid() {
		BalanceIsAllowCreditNotValid
				error =
				assertThrows(BalanceIsAllowCreditNotValid.class, () -> BalanceIsAllowCredit.create(null));
		assertThat(error.getMessage()).isEqualTo("IsAllowCredit is invalid");
	}

	@Test
	void it_should_throw_when_isExcluTotal_is_invalid() {
		BalanceIsExcluTotalNotValid
				error =
				assertThrows(BalanceIsExcluTotalNotValid.class, () -> BalanceIsExcluTotal.create(null));
		assertThat(error.getMessage()).isEqualTo("IsExcluTotal is invalid");
	}

	@Test
	void it_should_throw_when_activateNotification_is_invalid() {
		BalanceActivateNotificationNotValid
				error =
				assertThrows(BalanceActivateNotificationNotValid.class, () -> BalanceActivateNotification.create(null));
	...
	void it_should_throw_when_createdBy_is_invalid() {
		BalanceCreatedByNotValid
				error =
				assertThrows(BalanceCreatedByNotValid.class, () -> BalanceCreatedBy.create(""));
