...
class SaleUserGateTests {

@Mock
private SaleUserRepository repository;
@InjectMocks
private SaleUserGate gate;

private SaleUser sampleSaleUser;
...
sampleSaleUser = SaleUser.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleSaleUser.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleSaleUser.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleSaleUser.getId()))
	.thenReturn(Optional.of(sampleSaleUser));
	assertThat(gate.canRead(auth, sampleSaleUser.getId())).isEqualTo(expected);
	verify(repository).findById(sampleSaleUser.getId());
	}
	}
	...
	when(repository.findById(sampleSaleUser.getId()))
	.thenReturn(Optional.of(sampleSaleUser));
	assertThat(gate.canEdit(ownerAuth, sampleSaleUser.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleSaleUser.getId())).isTrue();
	}
	}
