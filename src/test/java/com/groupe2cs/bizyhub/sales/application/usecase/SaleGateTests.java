...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleSale.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleSale.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleSale.getId()))
	.thenReturn(Optional.of(sampleSale));
	assertThat(gate.canRead(auth, sampleSale.getId())).isEqualTo(expected);
	verify(repository).findById(sampleSale.getId());
	}
	}
	...
	when(repository.findById(sampleSale.getId()))
	.thenReturn(Optional.of(sampleSale));
	assertThat(gate.canEdit(ownerAuth, sampleSale.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleSale.getId())).isTrue();
	}
	}
