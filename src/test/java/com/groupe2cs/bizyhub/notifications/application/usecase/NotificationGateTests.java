...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleNotification.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleNotification.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleNotification.getId()))
	.thenReturn(Optional.of(sampleNotification));
	assertThat(gate.canRead(auth, sampleNotification.getId())).isEqualTo(expected);
	verify(repository).findById(sampleNotification.getId());
	}
	}
	...
	when(repository.findById(sampleNotification.getId()))
	.thenReturn(Optional.of(sampleNotification));
	assertThat(gate.canEdit(ownerAuth, sampleNotification.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleNotification.getId())).isTrue();
	}
	}
