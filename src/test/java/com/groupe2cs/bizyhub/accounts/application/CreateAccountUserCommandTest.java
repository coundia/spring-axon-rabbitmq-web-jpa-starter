...
public class CreateAccountUserCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateAccountUserCommand command = new CreateAccountUserCommand(
				...
				AccountUserIdentity.create(UUID.randomUUID().toString()),
				AccountUserPhone.create(UUID.randomUUID().toString()),
				AccountUserEmail.create(UUID.randomUUID().toString()),
				AccountUserRole.create(UUID.randomUUID().toString()),
				AccountUserStatus.create(UUID.randomUUID().toString()),
				AccountUserInvitedBy.create(UUID.randomUUID().toString()),
				AccountUserSyncAt.create(java.time.Instant.now().plusSeconds(3600)),
				AccountUserAcceptedAt.create(java.time.Instant.now().plusSeconds(3600)),
				AccountUserRevokedAt.create(java.time.Instant.now().plusSeconds(3600)),
				AccountUserMessage.create(UUID.randomUUID().toString()),
				AccountUserRemoteId.create(UUID.randomUUID().toString()),
				AccountUserLocalId.create(UUID.randomUUID().toString()),
