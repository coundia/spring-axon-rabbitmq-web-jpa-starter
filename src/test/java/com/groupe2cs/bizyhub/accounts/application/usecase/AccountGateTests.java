...
class AccountGateTests {
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleAccount.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleAccount.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleAccount.getId()))
					.thenReturn(Optional.of(sampleAccount));
			assertThat(gate.canRead(auth, sampleAccount.getId())).isEqualTo(expected);
			verify(repository).findById(sampleAccount.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleAccount.getId()))
				.thenReturn(Optional.of(sampleAccount));
		assertThat(gate.canEdit(ownerAuth, sampleAccount.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleAccount.getId())).isTrue();
	}
}
