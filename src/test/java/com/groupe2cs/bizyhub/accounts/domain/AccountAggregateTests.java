...
public class AccountAggregateTests extends BaseUnitTests {
	...
	void it_should_throw_when_balancePrev_is_invalid() {
		AccountBalancePrevNotValid
				error =
				assertThrows(AccountBalancePrevNotValid.class, () -> AccountBalancePrev.create(null));
		assertThat(error.getMessage()).isEqualTo("BalancePrev is invalid");
	}

	@Test
	void it_should_throw_when_balanceBlocked_is_invalid() {
		AccountBalanceBlockedNotValid
				error =
				assertThrows(AccountBalanceBlockedNotValid.class, () -> AccountBalanceBlocked.create(null));
		assertThat(error.getMessage()).isEqualTo("BalanceBlocked is invalid");
	}

	@Test
	void it_should_throw_when_balanceInit_is_invalid() {
		AccountBalanceInitNotValid
				error =
				assertThrows(AccountBalanceInitNotValid.class, () -> AccountBalanceInit.create(null));
		assertThat(error.getMessage()).isEqualTo("BalanceInit is invalid");
	}

	@Test
	void it_should_throw_when_balanceGoal_is_invalid() {
		AccountBalanceGoalNotValid
				error =
				assertThrows(AccountBalanceGoalNotValid.class, () -> AccountBalanceGoal.create(null));
		assertThat(error.getMessage()).isEqualTo("BalanceGoal is invalid");
	}

	@Test
	void it_should_throw_when_balanceLimit_is_invalid() {
		AccountBalanceLimitNotValid
				error =
				assertThrows(AccountBalanceLimitNotValid.class, () -> AccountBalanceLimit.create(null));
