...
public class AccountAggregateTests extends BaseUnitTests {
	...
	void it_should_create_account_with_valid_values() {
		...
		AccountCurrentBalance currentBalance = AccountCurrentBalance.create(585.54);
		AccountPreviousBalance previousBalance = AccountPreviousBalance.create(3524.8);
		...
		AccountAggregate
				aggregateAccountAggregate =
				new AccountAggregate(id,
						name,
						currency,
						currentBalance,
						previousBalance,
	...
	void it_should_throw_when_currentBalance_is_invalid() {
		AccountCurrentBalanceNotValid
				error =
				assertThrows(AccountCurrentBalanceNotValid.class, () -> AccountCurrentBalance.create(null));
		assertThat(error.getMessage()).isEqualTo("CurrentBalance is invalid");
	}

	@Test
	void it_should_throw_when_previousBalance_is_invalid() {
		AccountPreviousBalanceNotValid
				error =
				assertThrows(AccountPreviousBalanceNotValid.class, () -> AccountPreviousBalance.create(null));
	...
	void it_should_throw_when_updatedAt_is_invalid() {
		AccountUpdatedAtNotValid
				error =
				assertThrows(AccountUpdatedAtNotValid.class, () -> AccountUpdatedAt.create(null));
		assertThat(error.getMessage()).isEqualTo("UpdatedAt is invalid");
	}

	@Test
	void it_should_throw_when_reference_is_invalid() {
		AccountReferenceNotValid
				error =
				assertThrows(AccountReferenceNotValid.class, () -> AccountReference.create(""));
		assertThat(error.getMessage()).isEqualTo("Reference is invalid");
	}

	@Test
	void it_should_throw_when_createdBy_is_invalid() {
		AccountCreatedByNotValid
				error =
				assertThrows(AccountCreatedByNotValid.class, () -> AccountCreatedBy.create(""));
