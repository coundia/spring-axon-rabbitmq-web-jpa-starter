...
public class PasswordResetAggregateTests extends BaseUnitTests {
	...
	void it_should_create_passwordReset_with_valid_values() {
		...
		PasswordResetAggregate
				passwordReset =
				new PasswordResetAggregate(id, token, username, expiration, createdBy, tenant);
	...
	void it_should_throw_when_token_is_invalid() {
		PasswordResetTokenNotValid
				error =
				assertThrows(PasswordResetTokenNotValid.class, () -> PasswordResetToken.create(""));
		assertThat(error.getMessage()).isEqualTo("Token is invalid");
	}

	@Test
	void it_should_throw_when_username_is_invalid() {
		PasswordResetUsernameNotValid
				error =
				assertThrows(PasswordResetUsernameNotValid.class, () -> PasswordResetUsername.create(""));
		assertThat(error.getMessage()).isEqualTo("Username is invalid");
	}

	@Test
	void it_should_throw_when_expiration_is_invalid() {
		PasswordResetExpirationNotValid
				error =
	...
	void it_should_throw_when_tenant_is_invalid() {
		PasswordResetTenantNotValid
				error =
				assertThrows(PasswordResetTenantNotValid.class, () -> PasswordResetTenant.create(""));
