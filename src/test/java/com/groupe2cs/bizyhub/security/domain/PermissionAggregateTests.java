...
public class PermissionAggregateTests extends BaseUnitTests {
	...
	void it_should_create_permission_with_valid_values() {
		...
		PermissionAggregate permission = new PermissionAggregate(id, name, createdBy, tenant);
		...
		assertThat(permission.getTenant()).isEqualTo(tenant);
	}
	...
	void it_should_throw_when_tenant_is_invalid() {
		PermissionTenantNotValid
				error =
				assertThrows(PermissionTenantNotValid.class, () -> PermissionTenant.create(""));
