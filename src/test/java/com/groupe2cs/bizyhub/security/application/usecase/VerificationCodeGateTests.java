...
class VerificationCodeGateTests {

@Mock
private VerificationCodeRepository repository;
@InjectMocks
private VerificationCodeGate gate;

private VerificationCode sampleVerificationCode;
...
sampleVerificationCode = VerificationCode.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleVerificationCode.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleVerificationCode.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleVerificationCode.getId()))
	.thenReturn(Optional.of(sampleVerificationCode));
	assertThat(gate.canRead(auth, sampleVerificationCode.getId())).isEqualTo(expected);
	verify(repository).findById(sampleVerificationCode.getId());
	}
	}
	...
	when(repository.findById(sampleVerificationCode.getId()))
	.thenReturn(Optional.of(sampleVerificationCode));
	assertThat(gate.canEdit(ownerAuth, sampleVerificationCode.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleVerificationCode.getId())).isTrue();
	}
	}
