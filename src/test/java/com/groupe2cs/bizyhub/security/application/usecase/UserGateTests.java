...
@InjectMocks
private UserGate gate;

private CustomUser sampleCustomUser;
...
sampleCustomUser = CustomUser.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleCustomUser.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleCustomUser.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleCustomUser.getId()))
	.thenReturn(Optional.of(sampleCustomUser));
	assertThat(gate.canRead(auth, sampleCustomUser.getId())).isEqualTo(expected);
	verify(repository).findById(sampleCustomUser.getId());
	}
	}
	...
	when(repository.findById(sampleCustomUser.getId()))
	.thenReturn(Optional.of(sampleCustomUser));
	assertThat(gate.canEdit(ownerAuth, sampleCustomUser.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleCustomUser.getId())).isTrue();
	}
	}
