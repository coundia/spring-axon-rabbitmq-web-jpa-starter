...
class UserGateTests {
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleUser.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleUser.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleUser.getId()))
					.thenReturn(Optional.of(sampleUser));
			assertThat(gate.canRead(auth, sampleUser.getId())).isEqualTo(expected);
			verify(repository).findById(sampleUser.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleUser.getId()))
				.thenReturn(Optional.of(sampleUser));
		assertThat(gate.canEdit(ownerAuth, sampleUser.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleUser.getId())).isTrue();
	}
}
