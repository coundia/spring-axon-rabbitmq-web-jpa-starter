...
class RefreshTokenGateTests {

@Mock
private RefreshTokenRepository repository;
@InjectMocks
private RefreshTokenGate gate;
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleRefreshToken.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleRefreshToken.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleRefreshToken.getId()))
	.thenReturn(Optional.of(sampleRefreshToken));
	assertThat(gate.canRead(auth, sampleRefreshToken.getId())).isEqualTo(expected);
	verify(repository).findById(sampleRefreshToken.getId());
	}
	}
	...
	when(repository.findById(sampleRefreshToken.getId()))
	.thenReturn(Optional.of(sampleRefreshToken));
	assertThat(gate.canEdit(ownerAuth, sampleRefreshToken.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleRefreshToken.getId())).isTrue();
	}
	}
