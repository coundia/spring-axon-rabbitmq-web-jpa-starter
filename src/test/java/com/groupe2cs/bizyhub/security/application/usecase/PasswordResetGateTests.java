...
class PasswordResetGateTests {

	@Mock
	private PasswordResetRepository repository;
	@InjectMocks
	private PasswordResetGate gate;

	private PasswordReset samplePasswordReset;
	...
	void setUp() {
		...
		samplePasswordReset = PasswordReset.builder()
				.id("tx-123")
				.createdBy(user1)
				.tenant(tenantA)
				.build();
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, samplePasswordReset.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, samplePasswordReset.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(samplePasswordReset.getId()))
					.thenReturn(Optional.of(samplePasswordReset));
			assertThat(gate.canRead(auth, samplePasswordReset.getId())).isEqualTo(expected);
			verify(repository).findById(samplePasswordReset.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(samplePasswordReset.getId()))
				.thenReturn(Optional.of(samplePasswordReset));
		assertThat(gate.canEdit(ownerAuth, samplePasswordReset.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, samplePasswordReset.getId())).isTrue();
	}
}
