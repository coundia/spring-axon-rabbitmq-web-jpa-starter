...
class UserRoleGateTests {

	@Mock
	private UserRoleRepository repository;
	@InjectMocks
	private UserRoleGate gate;
	...
	void setUp() {
		...
		sampleUserRole = UserRole.builder()
				.id("tx-123")
				.createdBy(user1)
				.tenant(tenantA)
				.build();
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleUserRole.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleUserRole.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleUserRole.getId()))
					.thenReturn(Optional.of(sampleUserRole));
			assertThat(gate.canRead(auth, sampleUserRole.getId())).isEqualTo(expected);
			verify(repository).findById(sampleUserRole.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleUserRole.getId()))
				.thenReturn(Optional.of(sampleUserRole));
		assertThat(gate.canEdit(ownerAuth, sampleUserRole.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleUserRole.getId())).isTrue();
	}
}
