...
class RolePermissionGateTests {

@Mock
private RolePermissionRepository repository;
@InjectMocks
private RolePermissionGate gate;
...
sampleRolePermission = RolePermission.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleRolePermission.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleRolePermission.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleRolePermission.getId()))
	.thenReturn(Optional.of(sampleRolePermission));
	assertThat(gate.canRead(auth, sampleRolePermission.getId())).isEqualTo(expected);
	verify(repository).findById(sampleRolePermission.getId());
	}
	}
	...
	when(repository.findById(sampleRolePermission.getId()))
	.thenReturn(Optional.of(sampleRolePermission));
	assertThat(gate.canEdit(ownerAuth, sampleRolePermission.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleRolePermission.getId())).isTrue();
	}
	}
