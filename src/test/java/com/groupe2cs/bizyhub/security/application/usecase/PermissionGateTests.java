...
class PermissionGateTests {
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, samplePermission.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, samplePermission.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(samplePermission.getId()))
					.thenReturn(Optional.of(samplePermission));
			assertThat(gate.canRead(auth, samplePermission.getId())).isEqualTo(expected);
			verify(repository).findById(samplePermission.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(samplePermission.getId()))
				.thenReturn(Optional.of(samplePermission));
		assertThat(gate.canEdit(ownerAuth, samplePermission.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, samplePermission.getId())).isTrue();
	}
}
