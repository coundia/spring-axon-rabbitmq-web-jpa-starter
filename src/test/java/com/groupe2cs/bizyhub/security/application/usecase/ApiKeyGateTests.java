...
class ApiKeyGateTests {

@Mock
private ApiKeyRepository repository;
@InjectMocks
private ApiKeyGate gate;
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleApiKey.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleApiKey.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleApiKey.getId()))
	.thenReturn(Optional.of(sampleApiKey));
	assertThat(gate.canRead(auth, sampleApiKey.getId())).isEqualTo(expected);
	verify(repository).findById(sampleApiKey.getId());
	}
	}
	...
	when(repository.findById(sampleApiKey.getId()))
	.thenReturn(Optional.of(sampleApiKey));
	assertThat(gate.canEdit(ownerAuth, sampleApiKey.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleApiKey.getId())).isTrue();
	}
	}
