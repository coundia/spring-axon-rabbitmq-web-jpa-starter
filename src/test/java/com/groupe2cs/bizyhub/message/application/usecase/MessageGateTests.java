...
class MessageGateTests {

@Mock
private MessageRepository repository;
@InjectMocks
private MessageGate gate;
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleMessage.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleMessage.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleMessage.getId()))
	.thenReturn(Optional.of(sampleMessage));
	assertThat(gate.canRead(auth, sampleMessage.getId())).isEqualTo(expected);
	verify(repository).findById(sampleMessage.getId());
	}
	}
	...
	when(repository.findById(sampleMessage.getId()))
	.thenReturn(Optional.of(sampleMessage));
	assertThat(gate.canEdit(ownerAuth, sampleMessage.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleMessage.getId())).isTrue();
	}
	}
