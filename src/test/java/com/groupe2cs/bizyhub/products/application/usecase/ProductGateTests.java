...
class ProductGateTests {

	@Mock
	private ProductRepository repository;
	@InjectMocks
	private ProductGate gate;
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleProduct.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleProduct.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleProduct.getId()))
					.thenReturn(Optional.of(sampleProduct));
			assertThat(gate.canRead(auth, sampleProduct.getId())).isEqualTo(expected);
			verify(repository).findById(sampleProduct.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleProduct.getId()))
				.thenReturn(Optional.of(sampleProduct));
		assertThat(gate.canEdit(ownerAuth, sampleProduct.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleProduct.getId())).isTrue();
	}
}
