...
class CompanyGateTests {

@Mock
private CompanyRepository repository;
@InjectMocks
private CompanyGate gate;
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleCompany.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleCompany.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleCompany.getId()))
	.thenReturn(Optional.of(sampleCompany));
	assertThat(gate.canRead(auth, sampleCompany.getId())).isEqualTo(expected);
	verify(repository).findById(sampleCompany.getId());
	}
	}
	...
	when(repository.findById(sampleCompany.getId()))
	.thenReturn(Optional.of(sampleCompany));
	assertThat(gate.canEdit(ownerAuth, sampleCompany.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleCompany.getId())).isTrue();
	}
	}
