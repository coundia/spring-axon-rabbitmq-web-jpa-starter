...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleUnit.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleUnit.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleUnit.getId()))
	.thenReturn(Optional.of(sampleUnit));
	assertThat(gate.canRead(auth, sampleUnit.getId())).isEqualTo(expected);
	verify(repository).findById(sampleUnit.getId());
	}
	}
	...
	when(repository.findById(sampleUnit.getId()))
	.thenReturn(Optional.of(sampleUnit));
	assertThat(gate.canEdit(ownerAuth, sampleUnit.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleUnit.getId())).isTrue();
	}
	}
