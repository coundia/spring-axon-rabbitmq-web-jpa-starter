...
void it_should_create_transactionItem_with_valid_values() {
	...
	TransactionItemQuantity quantity = TransactionItemQuantity.create(47);
	...
	TransactionItemUnitPrice unitPrice = TransactionItemUnitPrice.create(5586.11);
	TransactionItemTotal total = TransactionItemTotal.create(6644.98);
	...
	TransactionItemAggregate aggregateTransactionItemAggregate = new TransactionItemAggregate(id, transaction, product, label, quantity, unit, account, remoteId, localId, syncAt, unitPrice, total, notes, createdBy, tenant);
