...
class TransactionUserGateTests {

	@Mock
	private TransactionUserRepository repository;
	@InjectMocks
	private TransactionUserGate gate;

	private TransactionUser sampleTransactionUser;
	...
	void setUp() {
		...
		sampleTransactionUser = TransactionUser.builder()
				.id("tx-123")
				.createdBy(user1)
				.tenant(tenantA)
				.build();
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleTransactionUser.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleTransactionUser.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleTransactionUser.getId()))
					.thenReturn(Optional.of(sampleTransactionUser));
			assertThat(gate.canRead(auth, sampleTransactionUser.getId())).isEqualTo(expected);
			verify(repository).findById(sampleTransactionUser.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleTransactionUser.getId()))
				.thenReturn(Optional.of(sampleTransactionUser));
		assertThat(gate.canEdit(ownerAuth, sampleTransactionUser.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleTransactionUser.getId())).isTrue();
	}
}
