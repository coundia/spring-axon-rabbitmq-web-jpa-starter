...
class TransactionItemGateTests {

@Mock
private TransactionItemRepository repository;
@InjectMocks
private TransactionItemGate gate;

private TransactionItem sampleTransactionItem;
...
sampleTransactionItem = TransactionItem.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleTransactionItem.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleTransactionItem.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleTransactionItem.getId()))
	.thenReturn(Optional.of(sampleTransactionItem));
	assertThat(gate.canRead(auth, sampleTransactionItem.getId())).isEqualTo(expected);
	verify(repository).findById(sampleTransactionItem.getId());
	}
	}
	...
	when(repository.findById(sampleTransactionItem.getId()))
	.thenReturn(Optional.of(sampleTransactionItem));
	assertThat(gate.canEdit(ownerAuth, sampleTransactionItem.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleTransactionItem.getId())).isTrue();
	}
	}
