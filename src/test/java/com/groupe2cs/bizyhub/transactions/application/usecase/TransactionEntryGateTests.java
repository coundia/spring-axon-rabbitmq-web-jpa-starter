...
class TransactionEntryGateTests {

@Mock
private TransactionEntryRepository repository;
@InjectMocks
private TransactionEntryGate gate;

private TransactionEntry sampleTransactionEntry;
...
sampleTransactionEntry = TransactionEntry.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleTransactionEntry.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleTransactionEntry.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleTransactionEntry.getId()))
	.thenReturn(Optional.of(sampleTransactionEntry));
	assertThat(gate.canRead(auth, sampleTransactionEntry.getId())).isEqualTo(expected);
	verify(repository).findById(sampleTransactionEntry.getId());
	}
	}
	...
	when(repository.findById(sampleTransactionEntry.getId()))
	.thenReturn(Optional.of(sampleTransactionEntry));
	assertThat(gate.canEdit(ownerAuth, sampleTransactionEntry.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleTransactionEntry.getId())).isTrue();
	}
	}
