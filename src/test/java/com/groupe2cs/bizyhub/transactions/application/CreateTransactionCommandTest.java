...
public class CreateTransactionCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateTransactionCommand command = new CreateTransactionCommand(
				...
				TransactionRemoteId.create(UUID.randomUUID().toString()),
				TransactionLocalId.create(UUID.randomUUID().toString()),
				TransactionCode.create(UUID.randomUUID().toString()),
				TransactionDescription.create(UUID.randomUUID().toString()),
				TransactionAmount.create(1139.84),
				TransactionTypeEntry.create(UUID.randomUUID().toString()),
				TransactionDateTransaction.create(java.time.Instant.now().plusSeconds(3600)),
				TransactionStatus.create(UUID.randomUUID().toString()),
				TransactionEntityName.create(UUID.randomUUID().toString()),
				TransactionEntityId.create(UUID.randomUUID().toString()),
				TransactionAccount.create(UUID.randomUUID().toString()),
				TransactionSyncAt.create(java.time.Instant.now().plusSeconds(3600)),
				TransactionCategory.create(UUID.randomUUID().toString()),
				TransactionCompany.create(UUID.randomUUID().toString()),
				TransactionCustomer.create(UUID.randomUUID().toString()),
				TransactionDebt.create(UUID.randomUUID().toString()),
