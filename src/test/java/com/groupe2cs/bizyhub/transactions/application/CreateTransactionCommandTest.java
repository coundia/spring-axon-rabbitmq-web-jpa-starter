...
public class CreateTransactionCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateTransactionCommand command = new CreateTransactionCommand(
				...
				TransactionAmount.create(7227.48),
				...
				TransactionIsRecurring.create(true),
				TransactionIsExcluToRapport.create(true),
				TransactionStatus.create(UUID.randomUUID().toString()),
				TransactionBalance.create(UUID.randomUUID().toString()),
				TransactionCategory.create(UUID.randomUUID().toString()),
				TransactionUpdatedAt.create(java.time.Instant.now().plusSeconds(3600)),
				TransactionCreatedAt.create(java.time.Instant.now().plusSeconds(3600)),
