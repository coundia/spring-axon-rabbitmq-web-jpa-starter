...
public class TransactionEntrySyncControllerTests extends BaseIntegrationTests {
...
private TransactionEntryRepository Repository;
	...
	@Test
	void it_should_initiate_sync_of_transactionEntrys() {
		TransactionEntrySyncRequest requestDTO = TransactionEntrySyncRequest.builder()
		.deltas(List.of(
...
.debt(com.groupe2cs.bizyhub.debts.infrastructure.entity.DebtFixtures.randomOneViaCommand(commandGateway,debtDataRepository, user).getId().value())
	...
	String uri = "/api/v1/commands/transactionEntry/sync";
	HttpEntity<TransactionEntrySyncRequest> request = new HttpEntity<>(requestDTO, headers);
		ResponseEntity<ApiResponseDto> response = testRestTemplate.postForEntity(uri, request, ApiResponseDto.class);
			...
			@Test
			void it_should_initiate_update_of_transactionEntrys() {

			String existingId = TransactionEntryFixtures.randomOneViaCommand(commandGateway,Repository, getCurrentUser()).getId().value();
...
.debt(com.groupe2cs.bizyhub.debts.infrastructure.entity.DebtFixtures.randomOneViaCommand(commandGateway,debtDataRepository, user).getId().value())
			...
			HttpEntity<TransactionEntrySyncRequest> request = new HttpEntity<>(requestDTO, headers);
				ResponseEntity<ApiResponseDto> response = testRestTemplate.postForEntity(uri, request, ApiResponseDto.class);
					...
					@Test
					void it_should_initiate_delete_of_transactionEntrys() {
					String existingId = TransactionEntryFixtures.randomOneViaCommand(commandGateway,Repository, getCurrentUser()).getId().value();
					TransactionEntrySyncRequest requestDTO = TransactionEntrySyncRequest.builder()
					.deltas(List.of(
					TransactionEntryDeltaDto.builder()
