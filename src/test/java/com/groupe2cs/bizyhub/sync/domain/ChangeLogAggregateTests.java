...
public class ChangeLogAggregateTests extends BaseUnitTests {
	...
	void it_should_create_changeLog_with_valid_values() {
		...
		ChangeLogAccount account = ChangeLogAccount.create(UUID.randomUUID().toString());
		...
		ChangeLogAttempts attempts = ChangeLogAttempts.create(78);
		...
		ChangeLogAggregate
				aggregateChangeLogAggregate =
				new ChangeLogAggregate(id,
						entityTable,
						account,
						remoteId,
						localId,
		...
		assertThat(aggregateChangeLogAggregate.getAccount()).isEqualTo(account);
	...
	void it_should_throw_when_entityTable_is_invalid() {
		ChangeLogEntityTableNotValid
				error =
				assertThrows(ChangeLogEntityTableNotValid.class, () -> ChangeLogEntityTable.create(""));
		assertThat(error.getMessage()).isEqualTo("EntityTable is invalid");
	}

	@Test
	void it_should_throw_when_attempts_is_invalid() {
		ChangeLogAttemptsNotValid
				error =
				assertThrows(ChangeLogAttemptsNotValid.class, () -> ChangeLogAttempts.create(-1));
