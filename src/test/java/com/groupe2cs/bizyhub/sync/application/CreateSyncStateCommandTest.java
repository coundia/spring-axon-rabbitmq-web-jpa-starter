...
public class CreateSyncStateCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateSyncStateCommand command = new CreateSyncStateCommand(
				SyncStateId.create(UUID.randomUUID().toString()),
				SyncStateEntityTable.create(UUID.randomUUID().toString()),
				SyncStateRemoteId.create(UUID.randomUUID().toString()),
				SyncStateLocalId.create(UUID.randomUUID().toString()),
				SyncStateAccount.create(UUID.randomUUID().toString()),
				SyncStateSyncAt.create(java.time.Instant.now().plusSeconds(3600)),
				SyncStateLastSyncAt.create(java.time.Instant.now().plusSeconds(3600)),
				SyncStateLastCursor.create(UUID.randomUUID().toString()),
				SyncStateCreatedBy.create(UUID.randomUUID().toString()),
				SyncStateTenant.create(UUID.randomUUID().toString())
		);
