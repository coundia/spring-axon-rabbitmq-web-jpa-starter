...
class SyncStateGateTests {

@Mock
private SyncStateRepository repository;
@InjectMocks
private SyncStateGate gate;

private SyncState sampleSyncState;
...
sampleSyncState = SyncState.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleSyncState.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleSyncState.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleSyncState.getId()))
	.thenReturn(Optional.of(sampleSyncState));
	assertThat(gate.canRead(auth, sampleSyncState.getId())).isEqualTo(expected);
	verify(repository).findById(sampleSyncState.getId());
	}
	}
	...
	when(repository.findById(sampleSyncState.getId()))
	.thenReturn(Optional.of(sampleSyncState));
	assertThat(gate.canEdit(ownerAuth, sampleSyncState.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleSyncState.getId())).isTrue();
	}
	}
