...
class ChangeLogGateTests {

@Mock
private ChangeLogRepository repository;
@InjectMocks
private ChangeLogGate gate;

private ChangeLog sampleChangeLog;
...
sampleChangeLog = ChangeLog.builder()
.id("tx-123")
.createdBy(user1)
.tenant(tenantA)
.build();
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleChangeLog.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleChangeLog.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleChangeLog.getId()))
	.thenReturn(Optional.of(sampleChangeLog));
	assertThat(gate.canRead(auth, sampleChangeLog.getId())).isEqualTo(expected);
	verify(repository).findById(sampleChangeLog.getId());
	}
	}
	...
	when(repository.findById(sampleChangeLog.getId()))
	.thenReturn(Optional.of(sampleChangeLog));
	assertThat(gate.canEdit(ownerAuth, sampleChangeLog.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleChangeLog.getId())).isTrue();
	}
	}
