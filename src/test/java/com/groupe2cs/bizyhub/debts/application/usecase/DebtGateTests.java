...
class DebtGateTests {
	...
	void canRead_variousScenarios(String scenario, boolean expected) {
		...
		if (auth == adminAuth) {
			assertThat(gate.canRead(auth, sampleDebt.getId())).isTrue();
			verifyNoInteractions(repository);
		} else if (!auth.isAuthenticated()) {
			assertThat(gate.canRead(auth, sampleDebt.getId())).isFalse();
			verifyNoInteractions(repository);
		} else {
			when(repository.findById(sampleDebt.getId()))
					.thenReturn(Optional.of(sampleDebt));
			assertThat(gate.canRead(auth, sampleDebt.getId())).isEqualTo(expected);
			verify(repository).findById(sampleDebt.getId());
		}
	}
	...
	void canEditAndDelete_delegateToCanRead() {
		when(repository.findById(sampleDebt.getId()))
				.thenReturn(Optional.of(sampleDebt));
		assertThat(gate.canEdit(ownerAuth, sampleDebt.getId())).isTrue();
		assertThat(gate.canDelete(ownerAuth, sampleDebt.getId())).isTrue();
	}
}
