...
class CustomerGateTests {

@Mock
private CustomerRepository repository;
@InjectMocks
private CustomerGate gate;
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleCustomer.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleCustomer.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleCustomer.getId()))
	.thenReturn(Optional.of(sampleCustomer));
	assertThat(gate.canRead(auth, sampleCustomer.getId())).isEqualTo(expected);
	verify(repository).findById(sampleCustomer.getId());
	}
	}
	...
	when(repository.findById(sampleCustomer.getId()))
	.thenReturn(Optional.of(sampleCustomer));
	assertThat(gate.canEdit(ownerAuth, sampleCustomer.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleCustomer.getId())).isTrue();
	}
	}
