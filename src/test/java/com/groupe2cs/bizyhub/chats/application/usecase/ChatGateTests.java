...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleChat.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleChat.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleChat.getId()))
	.thenReturn(Optional.of(sampleChat));
	assertThat(gate.canRead(auth, sampleChat.getId())).isEqualTo(expected);
	verify(repository).findById(sampleChat.getId());
	}
	}
	...
	when(repository.findById(sampleChat.getId()))
	.thenReturn(Optional.of(sampleChat));
	assertThat(gate.canEdit(ownerAuth, sampleChat.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleChat.getId())).isTrue();
	}
	}
