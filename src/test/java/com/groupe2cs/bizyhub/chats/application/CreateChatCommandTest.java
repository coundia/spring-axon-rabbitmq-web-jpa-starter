...
public class CreateChatCommandTest extends BaseUnitTests {
	...
	void it_should_send_command_to_command_gateway() {
		CreateChatCommand command = new CreateChatCommand(
				...
				ChatSyncAt.create(java.time.Instant.now().plusSeconds(3600)),
				ChatRemoteId.create(UUID.randomUUID().toString()),
				ChatLocalId.create(UUID.randomUUID().toString()),
				ChatAccount.create(UUID.randomUUID().toString()),
				ChatDateTransaction.create(java.time.Instant.now().plusSeconds(3600)),
