...
void it_should_create_category_with_valid_values() {
	...
	CategoryAggregate aggregateCategoryAggregate = new CategoryAggregate(id, reference, name, updatedAt, typeCategory, parentId, isDefault, icon, colorHex, createdBy, tenant);
	...
	assertThat(aggregateCategoryAggregate.getTenant()).isEqualTo(tenant);
		...
		CategoryTypeCategoryNotValid error = assertThrows(CategoryTypeCategoryNotValid.class, () -> CategoryTypeCategory.create(""));
		assertThat(error.getMessage()).isEqualTo("TypeCategory is invalid");
		}
		@Test
		void it_should_throw_when_parentId_is_invalid() {
		CategoryParentIdNotValid error = assertThrows(CategoryParentIdNotValid.class, () -> CategoryParentId.create(""));
		assertThat(error.getMessage()).isEqualTo("ParentId is invalid");
		}
		@Test
		void it_should_throw_when_isDefault_is_invalid() {
		CategoryIsDefaultNotValid error = assertThrows(CategoryIsDefaultNotValid.class, () -> CategoryIsDefault.create(null));
		...
		CategoryColorHexNotValid error = assertThrows(CategoryColorHexNotValid.class, () -> CategoryColorHex.create(""));
