...
void it_should_create_category_with_valid_values() {
    ...
    CategoryAccount account = CategoryAccount.create(UUID.randomUUID().toString());
    CategoryStatus status = CategoryStatus.create(UUID.randomUUID().toString());
    CategoryIsPublic isPublic = CategoryIsPublic.create(true);
    ...
    CategoryVersion version = CategoryVersion.create(44);
...
CategoryAggregate aggregateCategoryAggregate = new CategoryAggregate(id, code, name, remoteId, localId, account, status, isPublic, description, typeEntry, version, syncAt, createdBy, tenant);
	...
	assertThat(aggregateCategoryAggregate.getAccount()).isEqualTo(account);
	assertThat(aggregateCategoryAggregate.getStatus()).isEqualTo(status);
	assertThat(aggregateCategoryAggregate.getIsPublic()).isEqualTo(isPublic);
