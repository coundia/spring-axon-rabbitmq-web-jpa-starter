...
class CategoryGateTests {

@Mock
private CategoryRepository repository;
@InjectMocks
private CategoryGate gate;
...
static Stream<Arguments> readScenarios() {
	...
	if (auth == adminAuth) {
	assertThat(gate.canRead(auth, sampleCategory.getId())).isTrue();
	verifyNoInteractions(repository);
	} else if (!auth.isAuthenticated()) {
	assertThat(gate.canRead(auth, sampleCategory.getId())).isFalse();
	verifyNoInteractions(repository);
	} else {
	when(repository.findById(sampleCategory.getId()))
	.thenReturn(Optional.of(sampleCategory));
	assertThat(gate.canRead(auth, sampleCategory.getId())).isEqualTo(expected);
	verify(repository).findById(sampleCategory.getId());
	}
	}
	...
	when(repository.findById(sampleCategory.getId()))
	.thenReturn(Optional.of(sampleCategory));
	assertThat(gate.canEdit(ownerAuth, sampleCategory.getId())).isTrue();
	assertThat(gate.canDelete(ownerAuth, sampleCategory.getId())).isTrue();
	}
	}
