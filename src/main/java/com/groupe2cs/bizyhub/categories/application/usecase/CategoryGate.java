...
public class CategoryGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		if (opt.isEmpty()) {
			log.warn("canRead denied: Category {} not found", objectId);
			return false;
		}
		Category tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Category {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Category {}: {}",
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Category {}: {}",
				auth != null ? auth.getName() : null,
				objectId,
				result);
		return result;
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Category> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Category {} not found", objectId);
			return false;
		}
