...
@Tag(name = "Category Queries", description = "Endpoints for querying categorys by updatedAt")
@Slf4j
public class FindByUpdatedAtCategoryController {

private final CategoryReadApplicationService applicationService;

public FindByUpdatedAtCategoryController(CategoryReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/updatedAt")
@Operation(
summary = "Find category by updatedAt",
description = "Returns a list of categorys that match the given updatedAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CategoryResponse>> findByUpdatedAt(
	...
	var future = applicationService.findByCategoryUpdatedAt(CategoryUpdatedAt
	.create(updatedAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find category by updatedAt: {}", e.getMessage(), e);
