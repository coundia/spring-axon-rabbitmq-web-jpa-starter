...
@Tag(name = "Category Queries", description = "Endpoints for querying categorys by remoteId")
@Slf4j
public class FindByRemoteIdCategoryController {

private final CategoryReadApplicationService applicationService;

public FindByRemoteIdCategoryController(CategoryReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/remoteId")
@Operation(
summary = "Find category by remoteId",
description = "Returns a list of categorys that match the given remoteId"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CategoryResponse>> findByRemoteId(
	...
	var future = applicationService.findByCategoryRemoteId(CategoryRemoteId
	.create(remoteId) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find category by remoteId: {}", e.getMessage(), e);
