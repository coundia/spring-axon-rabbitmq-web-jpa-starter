...
public interface CategoryRepository extends JpaRepository<Category, String> {
	...
	Page<Category> findAllByTenantId(String tenantId, Pageable pageable);
       ...
       List<Category> findByIsActiveAndTenantId(Boolean isActive, String tenantId);
        ...
        @Query("SELECT e FROM Category e WHERE LOWER(e.reference) LIKE LOWER(CONCAT('%', :reference, '%')) AND e.createdBy.id = :createdById ORDER BY e.updatedAtAudit DESC, e.createdAtAudit  DESC")
        List<Category> findByReferenceAndCreatedById(String reference, String createdById);
        @Query("SELECT e FROM Category e WHERE LOWER(e.reference) LIKE LOWER(CONCAT('%', :reference, '%')) AND e.tenant.name = :tenantName ORDER BY e.updatedAtAudit DESC, e.createdAtAudit  DESC")
        List<Category> findByReferenceAndTenantName(String reference, String tenantName);

        @Query("SELECT e FROM Category e WHERE LOWER(e.reference) LIKE LOWER(CONCAT('%', :reference, '%')) AND e.tenant.id = :tenantId ORDER BY e.updatedAtAudit DESC, e.createdAtAudit  DESC")
