...
public interface CategoryRepository extends JpaRepository<Category, String> {
	...
	@Query("SELECT e FROM Category e WHERE e.typeCategoryRaw = ?1 and e.createdBy.id = ?2 ORDER BY e.createdAtAudit DESC")
	List<Category> findByTypeCategoryRawAndCreatedById(String typeCategoryRaw, String createdById);

	@Query("SELECT e FROM Category e WHERE e.typeCategoryRaw = ?1 and e.tenant.name = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByTypeCategoryRawAndTenantName(String typeCategoryRaw, String tenantName);

	@Query("SELECT e FROM Category e WHERE e.typeCategoryRaw = ?1 and e.tenant.id = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByTypeCategoryRawAndTenantId(String typeCategoryRaw, String tenantId);
	@Query("SELECT e FROM Category e WHERE e.details = ?1 and e.createdBy.id = ?2 ORDER BY e.createdAtAudit DESC")
	List<Category> findByDetailsAndCreatedById(String details, String createdById);

	@Query("SELECT e FROM Category e WHERE e.details = ?1 and e.tenant.name = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByDetailsAndTenantName(String details, String tenantName);

	@Query("SELECT e FROM Category e WHERE e.details = ?1 and e.tenant.id = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByDetailsAndTenantId(String details, String tenantId);
	@Query("SELECT e FROM Category e WHERE e.isActive = ?1 and e.createdBy.id = ?2 ORDER BY e.createdAtAudit DESC")
	List<Category> findByIsActiveAndCreatedById(Boolean isActive, String createdById);

	@Query("SELECT e FROM Category e WHERE e.isActive = ?1 and e.tenant.name = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByIsActiveAndTenantName(Boolean isActive, String tenantName);

	@Query("SELECT e FROM Category e WHERE e.isActive = ?1 and e.tenant.id = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByIsActiveAndTenantId(Boolean isActive, String tenantId);
	@Query("SELECT e FROM Category e WHERE e.updatedAt = ?1 and e.createdBy.id = ?2 ORDER BY e.createdAtAudit DESC")
	List<Category> findByUpdatedAtAndCreatedById(java.time.Instant updatedAt, String createdById);

	@Query("SELECT e FROM Category e WHERE e.updatedAt = ?1 and e.tenant.name = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByUpdatedAtAndTenantName(java.time.Instant updatedAt, String tenantName);

	@Query("SELECT e FROM Category e WHERE e.updatedAt = ?1 and e.tenant.id = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByUpdatedAtAndTenantId(java.time.Instant updatedAt, String tenantId);
	@Query("SELECT e FROM Category e WHERE e.reference = ?1 and e.createdBy.id = ?2 ORDER BY e.createdAtAudit DESC")
	List<Category> findByReferenceAndCreatedById(String reference, String createdById);

	@Query("SELECT e FROM Category e WHERE e.reference = ?1 and e.tenant.name = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByReferenceAndTenantName(String reference, String tenantName);

	@Query("SELECT e FROM Category e WHERE e.reference = ?1 and e.tenant.id = ?2 ORDER BY e.createdAtAudit DESC limit 1 ")
	Optional<Category> findByReferenceAndTenantId(String reference, String tenantId);
