...
public List<SettingResponse> handle(FindBySettingStringValueQuery query) {
	 ...
	 if(metaRequest.isAdmin()) {
	    entities = repository.findByStringValueAndTenantId(value, metaRequest.getTenantId());
	 }else{
	    entities = repository.findByStringValueAndCreatedById(value, metaRequest.getUserId());
	 }
