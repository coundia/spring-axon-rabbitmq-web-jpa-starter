...
public class SettingGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Setting> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Setting {} not found", objectId);
			return false;
		}
		Setting tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Setting {}",
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Setting> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Setting {} not found", objectId);
			return false;
		}
