...
@Tag(name = "Setting Queries", description = "Endpoints for querying settings by createdBy")
@Slf4j
public class FindByCreatedBySettingController {

private final SettingReadApplicationService applicationService;

public FindByCreatedBySettingController(SettingReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find setting by createdBy",
description = "Returns a list of settings that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SettingResponse>> findByCreatedBy(
	...
	var future = applicationService.findBySettingCreatedBy(SettingCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find setting by createdBy: {}", e.getMessage(), e);
