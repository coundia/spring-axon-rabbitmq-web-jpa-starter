...
public class CustomerGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Customer> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Customer {} not found", objectId);
			return false;
		}
		Customer tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Customer {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Customer {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Customer {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Customer> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Customer {} not found", objectId);
			return false;
		}
		Customer entity = opt.get();
		boolean result = entity.getCreatedBy() != null && userId.equals(entity.getCreatedBy().getId());
		log.info("canDelete result for userId {} on Customer {}: {}", userId, objectId, result);
		return result;
	}
