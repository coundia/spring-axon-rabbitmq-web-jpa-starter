...
import com.groupe2cs.bizyhub.customers.application.query.FindByCustomerRegionQuery;
...
public class FindByCustomerRegionHandler {

	private final CustomerRepository repository;

	@QueryHandler
	public List<CustomerResponse> handle(FindByCustomerRegionQuery query) {

		MetaRequest metaRequest = query.getMetaRequest();
		List<Customer> entities = null;
		String value = query.getRegion().value();

		if (metaRequest.isAdmin()) {
			entities = repository.findByRegionAndTenantId(value, metaRequest.getTenantId());
		} else {
			entities = repository.findByRegionAndCreatedById(value, metaRequest.getUserId());
		}
