...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by tenant")
@Slf4j
public class FindByTenantCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByTenantCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find customer by tenant",
description = "Returns a list of customers that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByTenant(
	...
	var future = applicationService.findByCustomerTenant(CustomerTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by tenant: {}", e.getMessage(), e);
