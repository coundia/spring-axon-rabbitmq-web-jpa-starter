...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by remoteId")
@Slf4j
public class FindByRemoteIdCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByRemoteIdCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/remoteId")
@Operation(
summary = "Find customer by remoteId",
description = "Returns a list of customers that match the given remoteId"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByRemoteId(
	...
	var future = applicationService.findByCustomerRemoteId(CustomerRemoteId
	.create(remoteId) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by remoteId: {}", e.getMessage(), e);
