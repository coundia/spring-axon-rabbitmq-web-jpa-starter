...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by region")
@Slf4j
public class FindByRegionCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByRegionCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/region")
@Operation(
summary = "Find customer by region",
description = "Returns a list of customers that match the given region"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByRegion(
	...
	var future = applicationService.findByCustomerRegion(CustomerRegion
	.create(region) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by region: {}", e.getMessage(), e);
