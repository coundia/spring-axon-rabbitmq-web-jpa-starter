...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by balance")
@Slf4j
public class FindByBalanceCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByBalanceCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/balance")
@Operation(
summary = "Find customer by balance",
description = "Returns a list of customers that match the given balance"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByBalance(
	...
	var future = applicationService.findByCustomerBalance(CustomerBalance
	.create(balance) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by balance: {}", e.getMessage(), e);
