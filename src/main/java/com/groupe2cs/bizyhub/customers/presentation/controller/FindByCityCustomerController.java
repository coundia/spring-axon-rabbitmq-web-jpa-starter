...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by city")
@Slf4j
public class FindByCityCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByCityCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/city")
@Operation(
summary = "Find customer by city",
description = "Returns a list of customers that match the given city"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByCity(
	@AuthenticationPrincipal Jwt jwt,
	@Parameter(description = "Value of the city to filter by", required = true)
	...
	var future = applicationService.findByCustomerCity(CustomerCity
	.create(city) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by city: {}", e.getMessage(), e);
