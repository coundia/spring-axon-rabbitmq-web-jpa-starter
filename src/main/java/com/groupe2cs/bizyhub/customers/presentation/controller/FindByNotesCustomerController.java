...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by notes")
@Slf4j
public class FindByNotesCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByNotesCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/notes")
@Operation(
summary = "Find customer by notes",
description = "Returns a list of customers that match the given notes"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByNotes(
	...
	var future = applicationService.findByCustomerNotes(CustomerNotes
	.create(notes) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by notes: {}", e.getMessage(), e);
