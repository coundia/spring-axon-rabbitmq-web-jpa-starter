...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by phone")
@Slf4j
public class FindByPhoneCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByPhoneCustomerController(CustomerReadApplicationService  applicationService) {
...
description = "Returns a list of customers that match the given phone"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByPhone(
	...
	var future = applicationService.findByCustomerPhone(CustomerPhone
	.create(phone) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by phone: {}", e.getMessage(), e);
