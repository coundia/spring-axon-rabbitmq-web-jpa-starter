...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by lastName")
@Slf4j
public class FindByLastNameCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByLastNameCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/lastName")
@Operation(
summary = "Find customer by lastName",
description = "Returns a list of customers that match the given lastName"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByLastName(
	...
	@Parameter(description = "Value of the lastName to filter by", required = true)
	...
	var future = applicationService.findByCustomerLastName(CustomerLastName
	.create(lastName) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by lastName: {}", e.getMessage(), e);
