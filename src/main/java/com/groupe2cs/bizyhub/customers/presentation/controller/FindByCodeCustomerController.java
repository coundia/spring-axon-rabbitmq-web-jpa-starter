...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by code")
@Slf4j
public class FindByCodeCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByCodeCustomerController(CustomerReadApplicationService  applicationService) {
...
description = "Returns a list of customers that match the given code"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByCode(
	...
	var future = applicationService.findByCustomerCode(CustomerCode
	.create(code) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by code: {}", e.getMessage(), e);
