...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by createdBy")
@Slf4j
public class FindByCreatedByCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByCreatedByCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find customer by createdBy",
description = "Returns a list of customers that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByCreatedBy(
	...
	var future = applicationService.findByCustomerCreatedBy(CustomerCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by createdBy: {}", e.getMessage(), e);
