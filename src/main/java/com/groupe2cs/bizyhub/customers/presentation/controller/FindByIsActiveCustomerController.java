...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by isActive")
@Slf4j
public class FindByIsActiveCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByIsActiveCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/isActive")
@Operation(
summary = "Find customer by isActive",
description = "Returns a list of customers that match the given isActive"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByIsActive(
	...
	var future = applicationService.findByCustomerIsActive(CustomerIsActive
	.create(isActive) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by isActive: {}", e.getMessage(), e);
