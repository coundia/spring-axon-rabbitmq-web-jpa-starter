...
@Tag(name = "Customer Queries", description = "Endpoints for querying customers by balanceDebt")
@Slf4j
public class FindByBalanceDebtCustomerController {

private final CustomerReadApplicationService applicationService;

public FindByBalanceDebtCustomerController(CustomerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/balanceDebt")
@Operation(
summary = "Find customer by balanceDebt",
description = "Returns a list of customers that match the given balanceDebt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CustomerResponse>> findByBalanceDebt(
	...
	var future = applicationService.findByCustomerBalanceDebt(CustomerBalanceDebt
	.create(balanceDebt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find customer by balanceDebt: {}", e.getMessage(), e);
