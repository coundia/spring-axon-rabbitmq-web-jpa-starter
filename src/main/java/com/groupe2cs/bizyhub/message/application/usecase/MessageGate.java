...
public class MessageGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Message> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Message {} not found", objectId);
			return false;
		}
		Message tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Message {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Message {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Message {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Message> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Message {} not found", objectId);
			return false;
		}
		Message entity = opt.get();


		boolean result = entity.getCreatedBy() != null && userId.equals(entity.getCreatedBy().getId());
		log.info("canDelete result for userId {} on Message {}: {}", userId, objectId, result);
		return result;
	}
