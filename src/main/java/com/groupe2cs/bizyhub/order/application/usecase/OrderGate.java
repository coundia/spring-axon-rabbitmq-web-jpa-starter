...
public class OrderGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Order> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Order {} not found", objectId);
			return false;
		}
		Order tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Order {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Order {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Order {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Order> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Order {} not found", objectId);
			return false;
		}
		...
		log.info("canDelete result for userId {} on Order {}: {}", userId, objectId, result);
		return result;
	}
