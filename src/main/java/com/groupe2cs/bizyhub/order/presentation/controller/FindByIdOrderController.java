...
@PreAuthorize("@orderGate.canRead(authentication, #id)")
@RestController
@RequestMapping("/api/v1/queries/orders")
@Tag(name = "Order Queries", description = "Endpoints for querying orders by id")
@Slf4j
public class FindByIdOrderController {

	private final OrderReadApplicationService applicationService;

	public FindByIdOrderController(OrderReadApplicationService applicationService) {
	...
	@Operation(
			...
			description = "Returns a single orders that match the given id"
	)
	@ApiResponses(value = {
	...
	public ResponseEntity<OrderResponse> findById(
		...
		try {
			...
			var future = applicationService.findByOrderId(OrderId
					.create(id), metaRequest);

			return ResponseEntity.ok(future);
		} catch (Exception e) {
			log.error("Failed to find order by id: {}", e.getMessage(), e);
