...
@PreAuthorize("@unitGate.canRead(authentication, #id)")
@RestController
@RequestMapping("/api/v1/queries/unit")
@Tag(name = "Unit Queries", description = "Endpoints for querying units by id")
@Slf4j
public class FindByIdUnitController {

private final UnitReadApplicationService applicationService;

public FindByIdUnitController(UnitReadApplicationService  applicationService) {
...
description = "Returns a single units that match the given id"
)
@ApiResponses(value = {
...
public ResponseEntity<UnitResponse> findById(
	...
	var future = applicationService.findByUnitId(UnitId
	.create(id) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find unit by id: {}", e.getMessage(), e);
