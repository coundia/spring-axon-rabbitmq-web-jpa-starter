...
@Tag(name = "Unit Queries", description = "Endpoints for querying units by syncAt")
@Slf4j
public class FindBySyncAtUnitController {

private final UnitReadApplicationService applicationService;

public FindBySyncAtUnitController(UnitReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/syncAt")
@Operation(
summary = "Find unit by syncAt",
description = "Returns a list of units that match the given syncAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<UnitResponse>> findBySyncAt(
	...
	var future = applicationService.findByUnitSyncAt(UnitSyncAt
	.create(syncAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find unit by syncAt: {}", e.getMessage(), e);
