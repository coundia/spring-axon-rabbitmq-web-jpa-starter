...
@Tag(name = "Unit Queries", description = "Endpoints for querying units by tenant")
@Slf4j
public class FindByTenantUnitController {

private final UnitReadApplicationService applicationService;

public FindByTenantUnitController(UnitReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find unit by tenant",
description = "Returns a list of units that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<UnitResponse>> findByTenant(
	...
	var future = applicationService.findByUnitTenant(UnitTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find unit by tenant: {}", e.getMessage(), e);
