...
@Tag(name = "Unit Queries", description = "Endpoints for querying units by name")
@Slf4j
public class FindByNameUnitController {

private final UnitReadApplicationService applicationService;

public FindByNameUnitController(UnitReadApplicationService  applicationService) {
...
description = "Returns a list of units that match the given name"
)
@ApiResponses(value = {
...
public ResponseEntity<List<UnitResponse>> findByName(
	...
	var future = applicationService.findByUnitName(UnitName
	.create(name) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find unit by name: {}", e.getMessage(), e);
