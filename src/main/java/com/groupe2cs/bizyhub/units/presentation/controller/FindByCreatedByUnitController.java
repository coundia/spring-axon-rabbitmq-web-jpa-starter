...
@Tag(name = "Unit Queries", description = "Endpoints for querying units by createdBy")
@Slf4j
public class FindByCreatedByUnitController {

private final UnitReadApplicationService applicationService;

public FindByCreatedByUnitController(UnitReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find unit by createdBy",
description = "Returns a list of units that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<UnitResponse>> findByCreatedBy(
	...
	var future = applicationService.findByUnitCreatedBy(UnitCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find unit by createdBy: {}", e.getMessage(), e);
