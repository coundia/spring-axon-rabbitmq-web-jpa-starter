...
@PreAuthorize("@unitGate.canCreate(authentication)")
...
private final UnitCreateApplicationService applicationService;

public AddUnitController(UnitCreateApplicationService applicationService) {
...
description = "Unit request payload",
required = true,
...
public ResponseEntity<UnitResponse> addUnit(@Valid @RequestBody UnitRequest request,
	...
	UnitResponse response =  applicationService.createUnit(
			request,
			metaRequest
	);
	...
	//e.printStackTrace();
	log.error("Failed to create unit: {}", ex.getMessage());
