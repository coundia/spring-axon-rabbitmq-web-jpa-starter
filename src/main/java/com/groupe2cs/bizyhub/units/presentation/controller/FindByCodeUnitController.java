...
@Tag(name = "Unit Queries", description = "Endpoints for querying units by code")
@Slf4j
public class FindByCodeUnitController {

private final UnitReadApplicationService applicationService;

public FindByCodeUnitController(UnitReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/code")
@Operation(
summary = "Find unit by code",
description = "Returns a list of units that match the given code"
)
@ApiResponses(value = {
...
public ResponseEntity<List<UnitResponse>> findByCode(
	...
	var future = applicationService.findByUnitCode(UnitCode
	.create(code) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find unit by code: {}", e.getMessage(), e);
