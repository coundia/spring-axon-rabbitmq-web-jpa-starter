...
public class UnitGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Unit> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Unit {} not found", objectId);
			return false;
		}
		Unit tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Unit {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Unit {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Unit {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Unit> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Unit {} not found", objectId);
			return false;
		}
		Unit entity = opt.get();
		boolean result = entity.getCreatedBy() != null && userId.equals(entity.getCreatedBy().getId());
		log.info("canDelete result for userId {} on Unit {}: {}", userId, objectId, result);
		return result;
	}
