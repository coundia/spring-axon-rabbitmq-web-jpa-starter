...
public class ProductGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Product> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Product {} not found", objectId);
			return false;
		}
		Product tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Product {}",
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Product> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Product {} not found", objectId);
			return false;
		}
		...
		log.info("canDelete result for userId {} on Product {}: {}", userId, objectId, result);
		return result;
	}
