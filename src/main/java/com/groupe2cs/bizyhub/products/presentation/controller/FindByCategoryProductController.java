...
@Tag(name = "Product Queries", description = "Endpoints for querying products by category")
@Slf4j
public class FindByCategoryProductController {

private final ProductReadApplicationService applicationService;

public FindByCategoryProductController(ProductReadApplicationService  applicationService) {
...
description = "Returns a list of products that match the given category"
)
@ApiResponses(value = {
...
public ResponseEntity<List<ProductResponse>> findByCategory(
	...
	var future = applicationService.findByProductCategory(ProductCategory
	.create(category) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find product by category: {}", e.getMessage(), e);
