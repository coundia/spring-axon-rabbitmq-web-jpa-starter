...
@Tag(name = "Product Queries", description = "Endpoints for querying products by description")
@Slf4j
public class FindByDescriptionProductController {

private final ProductReadApplicationService applicationService;

public FindByDescriptionProductController(ProductReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/description")
@Operation(
summary = "Find product by description",
...
public ResponseEntity<List<ProductResponse>> findByDescription(
	...
	var future = applicationService.findByProductDescription(ProductDescription
	.create(description) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find product by description: {}", e.getMessage(), e);
