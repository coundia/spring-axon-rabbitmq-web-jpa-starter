...
@Tag(name = "Product Queries", description = "Endpoints for querying products by code")
@Slf4j
public class FindByCodeProductController {

private final ProductReadApplicationService applicationService;

public FindByCodeProductController(ProductReadApplicationService  applicationService) {
...
description = "Returns a list of products that match the given code"
)
@ApiResponses(value = {
...
public ResponseEntity<List<ProductResponse>> findByCode(
	...
	var future = applicationService.findByProductCode(ProductCode
	.create(code) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find product by code: {}", e.getMessage(), e);
