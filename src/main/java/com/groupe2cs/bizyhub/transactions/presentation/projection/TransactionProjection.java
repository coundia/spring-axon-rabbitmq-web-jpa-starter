...
Transaction entity = Transaction.builder()
		.id(event.getId().value())
 		...
 		.status(event.getStatus().value())
  		.balance(new com.groupe2cs.bizyhub.balances.infrastructure.entity.Balance(event.getBalance().value()))
 		.category(new com.groupe2cs.bizyhub.categories.infrastructure.entity.Category(event.getCategory().value()))
...
.orElseThrow(() -> new RuntimeException("Transaction not found"));
		...
		entity.setBalance(new com.groupe2cs.bizyhub.balances.infrastructure.entity.Balance(event.getBalance().value()));
