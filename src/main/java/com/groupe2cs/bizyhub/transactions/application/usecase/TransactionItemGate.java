...
public class TransactionItemGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<TransactionItem> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: TransactionItem {} not found", objectId);
			return false;
		}
		TransactionItem tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for TransactionItem {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on TransactionItem {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on TransactionItem {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<TransactionItem> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: TransactionItem {} not found", objectId);
			return false;
		}
