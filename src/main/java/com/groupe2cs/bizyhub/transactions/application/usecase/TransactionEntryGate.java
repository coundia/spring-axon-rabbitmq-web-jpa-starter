...
public class TransactionEntryGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<TransactionEntry> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: TransactionEntry {} not found", objectId);
			return false;
		}
		TransactionEntry tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for TransactionEntry {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on TransactionEntry {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on TransactionEntry {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<TransactionEntry> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: TransactionEntry {} not found", objectId);
			return false;
		}
		TransactionEntry entity = opt.get();
		boolean result = entity.getCreatedBy() != null && userId.equals(entity.getCreatedBy().getId());
		log.info("canDelete result for userId {} on TransactionEntry {}: {}", userId, objectId, result);
		return result;
	}
