...
Optional<Sale> opt = repository.findById(objectId);
	if (opt.isEmpty()) {
	...
	log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Sale {}", currentUserId, owner, sameTenant, result, objectId);
	return result;
	}
	...
	log.info("canEdit result for user {} on Sale {}: {}", auth != null ? auth.getName() : null, objectId, result);
	return result;
	}
	...
	boolean result = canRead(auth, objectId);
	log.info("canDelete result for user {} on Sale {}: {}", auth != null ? auth.getName() : null, objectId, result);
	return result;
	}
