...
@Tag(name = "Sale Queries", description = "Endpoints for querying sales by category")
@Slf4j
public class FindByCategorySaleController {

private final SaleReadApplicationService applicationService;

public FindByCategorySaleController(SaleReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/category")
@Operation(
summary = "Find sale by category",
description = "Returns a list of sales that match the given category"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleResponse>> findByCategory(
	...
	var future = applicationService.findBySaleCategory(SaleCategory
	.create(category) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find sale by category: {}", e.getMessage(), e);
