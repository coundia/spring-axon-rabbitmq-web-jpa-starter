...
@Tag(name = "Sale Queries", description = "Endpoints for querying sales by name")
@Slf4j
public class FindByNameSaleController {

private final SaleReadApplicationService applicationService;

public FindByNameSaleController(SaleReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/name")
@Operation(
summary = "Find sale by name",
description = "Returns a list of sales that match the given name"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleResponse>> findByName(
	...
	var future = applicationService.findBySaleName(SaleName
	.create(name) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find sale by name: {}", e.getMessage(), e);
