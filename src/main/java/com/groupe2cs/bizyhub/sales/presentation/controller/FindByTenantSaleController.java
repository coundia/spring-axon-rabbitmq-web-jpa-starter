...
@Tag(name = "Sale Queries", description = "Endpoints for querying sales by tenant")
@Slf4j
public class FindByTenantSaleController {

private final SaleReadApplicationService applicationService;

public FindByTenantSaleController(SaleReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find sale by tenant",
description = "Returns a list of sales that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleResponse>> findByTenant(
	...
	var future = applicationService.findBySaleTenant(SaleTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find sale by tenant: {}", e.getMessage(), e);
