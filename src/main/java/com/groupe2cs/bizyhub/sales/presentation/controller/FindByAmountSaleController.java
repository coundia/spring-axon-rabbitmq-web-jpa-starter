...
@Tag(name = "Sale Queries", description = "Endpoints for querying sales by amount")
@Slf4j
public class FindByAmountSaleController {

private final SaleReadApplicationService applicationService;

public FindByAmountSaleController(SaleReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/amount")
@Operation(
summary = "Find sale by amount",
description = "Returns a list of sales that match the given amount"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleResponse>> findByAmount(
	...
	var future = applicationService.findBySaleAmount(SaleAmount
	.create(amount) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find sale by amount: {}", e.getMessage(), e);
