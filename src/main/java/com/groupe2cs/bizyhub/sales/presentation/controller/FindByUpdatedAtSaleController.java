...
@Tag(name = "Sale Queries", description = "Endpoints for querying sales by updatedAt")
@Slf4j
public class FindByUpdatedAtSaleController {

private final SaleReadApplicationService applicationService;

public FindByUpdatedAtSaleController(SaleReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/updatedAt")
@Operation(
summary = "Find sale by updatedAt",
description = "Returns a list of sales that match the given updatedAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleResponse>> findByUpdatedAt(
	...
	var future = applicationService.findBySaleUpdatedAt(SaleUpdatedAt
	.create(updatedAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find sale by updatedAt: {}", e.getMessage(), e);
