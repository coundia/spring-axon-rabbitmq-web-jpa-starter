...
@Tag(name = "SaleUser Queries", description = "Endpoints for querying saleUsers by details")
@Slf4j
public class FindByDetailsSaleUserController {

private final SaleUserReadApplicationService applicationService;

public FindByDetailsSaleUserController(SaleUserReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/details")
@Operation(
summary = "Find saleUser by details",
description = "Returns a list of saleUsers that match the given details"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleUserResponse>> findByDetails(
	...
	var future = applicationService.findBySaleUserDetails(SaleUserDetails
	.create(details) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find saleUser by details: {}", e.getMessage(), e);
