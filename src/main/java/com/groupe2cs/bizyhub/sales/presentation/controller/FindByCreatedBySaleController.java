...
@Tag(name = "Sale Queries", description = "Endpoints for querying sales by createdBy")
@Slf4j
public class FindByCreatedBySaleController {

private final SaleReadApplicationService applicationService;

public FindByCreatedBySaleController(SaleReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find sale by createdBy",
description = "Returns a list of sales that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleResponse>> findByCreatedBy(
	...
	var future = applicationService.findBySaleCreatedBy(SaleCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find sale by createdBy: {}", e.getMessage(), e);
