...
@Tag(name = "Sale Queries", description = "Endpoints for querying sales by account")
@Slf4j
public class FindByAccountSaleController {

private final SaleReadApplicationService applicationService;

public FindByAccountSaleController(SaleReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/account")
@Operation(
summary = "Find sale by account",
description = "Returns a list of sales that match the given account"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleResponse>> findByAccount(
	...
	var future = applicationService.findBySaleAccount(SaleAccount
	.create(account) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find sale by account: {}", e.getMessage(), e);
