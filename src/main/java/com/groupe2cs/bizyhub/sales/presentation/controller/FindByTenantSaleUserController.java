...
@Tag(name = "SaleUser Queries", description = "Endpoints for querying saleUsers by tenant")
@Slf4j
public class FindByTenantSaleUserController {

private final SaleUserReadApplicationService applicationService;

public FindByTenantSaleUserController(SaleUserReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find saleUser by tenant",
description = "Returns a list of saleUsers that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleUserResponse>> findByTenant(
	...
	var future = applicationService.findBySaleUserTenant(SaleUserTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find saleUser by tenant: {}", e.getMessage(), e);
