...
@Tag(name = "SaleUser Queries", description = "Endpoints for querying saleUsers by name")
@Slf4j
public class FindByNameSaleUserController {

private final SaleUserReadApplicationService applicationService;

public FindByNameSaleUserController(SaleUserReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/name")
@Operation(
summary = "Find saleUser by name",
description = "Returns a list of saleUsers that match the given name"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleUserResponse>> findByName(
	...
	var future = applicationService.findBySaleUserName(SaleUserName
	.create(name) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find saleUser by name: {}", e.getMessage(), e);
