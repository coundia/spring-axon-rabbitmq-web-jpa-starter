...
@Tag(name = "SaleUser Queries", description = "Endpoints for querying saleUsers by updatedAt")
@Slf4j
public class FindByUpdatedAtSaleUserController {

private final SaleUserReadApplicationService applicationService;

public FindByUpdatedAtSaleUserController(SaleUserReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/updatedAt")
@Operation(
summary = "Find saleUser by updatedAt",
description = "Returns a list of saleUsers that match the given updatedAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SaleUserResponse>> findByUpdatedAt(
	...
	var future = applicationService.findBySaleUserUpdatedAt(SaleUserUpdatedAt
	.create(updatedAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find saleUser by updatedAt: {}", e.getMessage(), e);
