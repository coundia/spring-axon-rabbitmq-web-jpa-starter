...
@Tag(name = "Chat Queries", description = "Endpoints for querying chats by syncAt")
@Slf4j
public class FindBySyncAtChatController {

private final ChatReadApplicationService applicationService;

public FindBySyncAtChatController(ChatReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/syncAt")
@Operation(
summary = "Find chat by syncAt",
description = "Returns a list of chats that match the given syncAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<ChatResponse>> findBySyncAt(
	...
	var future = applicationService.findByChatSyncAt(ChatSyncAt
	.create(syncAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find chat by syncAt: {}", e.getMessage(), e);
