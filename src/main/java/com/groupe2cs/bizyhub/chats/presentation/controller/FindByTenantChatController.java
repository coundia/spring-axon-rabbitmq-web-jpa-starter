...
@Tag(name = "Chat Queries", description = "Endpoints for querying chats by tenant")
@Slf4j
public class FindByTenantChatController {

private final ChatReadApplicationService applicationService;

public FindByTenantChatController(ChatReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find chat by tenant",
description = "Returns a list of chats that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<ChatResponse>> findByTenant(
	...
	var future = applicationService.findByChatTenant(ChatTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find chat by tenant: {}", e.getMessage(), e);
