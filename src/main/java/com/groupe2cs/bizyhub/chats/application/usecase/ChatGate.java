...
public class ChatGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Chat> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Chat {} not found", objectId);
			return false;
		}
		Chat tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Chat {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Chat {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Chat {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Chat> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Chat {} not found", objectId);
			return false;
		}
		Chat entity = opt.get();
