...
public List<ChatResponse> findByChatUpdatedAt(
	ChatUpdatedAt value,
	MetaRequest metaRequest
	) {

	FindByChatUpdatedAtQuery query = new FindByChatUpdatedAtQuery(value,metaRequest);
	CompletableFuture<List<ChatResponse>> future = queryGateway.query(query,
	org.axonframework.messaging.responsetypes.ResponseTypes.multipleInstancesOf(ChatResponse.class));
	return future.join();
}
public List<ChatResponse> findByChatReference(
	ChatReference value,
	MetaRequest metaRequest
	) {

	FindByChatReferenceQuery query = new FindByChatReferenceQuery(value,metaRequest);
	CompletableFuture<List<ChatResponse>> future = queryGateway.query(query,
	org.axonframework.messaging.responsetypes.ResponseTypes.multipleInstancesOf(ChatResponse.class));
	return future.join();
}
