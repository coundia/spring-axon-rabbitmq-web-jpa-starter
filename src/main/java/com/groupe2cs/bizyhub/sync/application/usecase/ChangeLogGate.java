...
public class ChangeLogGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<ChangeLog> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: ChangeLog {} not found", objectId);
			return false;
		}
		ChangeLog tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for ChangeLog {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on ChangeLog {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on ChangeLog {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<ChangeLog> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: ChangeLog {} not found", objectId);
			return false;
		}
