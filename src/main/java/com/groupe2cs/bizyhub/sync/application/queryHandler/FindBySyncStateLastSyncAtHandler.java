...
import com.groupe2cs.bizyhub.sync.application.query.FindBySyncStateLastSyncAtQuery;
...
public class FindBySyncStateLastSyncAtHandler {

	private final SyncStateRepository repository;

	@QueryHandler
	public List<SyncStateResponse> handle(FindBySyncStateLastSyncAtQuery query) {

		MetaRequest metaRequest = query.getMetaRequest();
		List<SyncState> entities = null;
		java.time.Instant value = query.getLastSyncAt().value();

		if (metaRequest.isAdmin()) {
			entities = repository.findByLastSyncAtAndTenantId(value, metaRequest.getTenantId());
		} else {
			entities = repository.findByLastSyncAtAndCreatedById(value, metaRequest.getUserId());
		}
