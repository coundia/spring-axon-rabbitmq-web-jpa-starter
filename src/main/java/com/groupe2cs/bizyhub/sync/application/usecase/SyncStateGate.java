...
public class SyncStateGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<SyncState> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: SyncState {} not found", objectId);
			return false;
		}
		SyncState tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for SyncState {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on SyncState {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on SyncState {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<SyncState> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: SyncState {} not found", objectId);
			return false;
		}
