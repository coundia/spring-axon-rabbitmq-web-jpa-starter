...
@Tag(name = "ChangeLog Queries", description = "Endpoints for querying changeLogs by createdBy")
@Slf4j
public class FindByCreatedByChangeLogController {

private final ChangeLogReadApplicationService applicationService;

public FindByCreatedByChangeLogController(ChangeLogReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find changeLog by createdBy",
description = "Returns a list of changeLogs that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<ChangeLogResponse>> findByCreatedBy(
	...
	var future = applicationService.findByChangeLogCreatedBy(ChangeLogCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find changeLog by createdBy: {}", e.getMessage(), e);
