...
@Tag(name = "SyncState Queries", description = "Endpoints for querying syncStates by createdBy")
@Slf4j
public class FindByCreatedBySyncStateController {

private final SyncStateReadApplicationService applicationService;

public FindByCreatedBySyncStateController(SyncStateReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find syncState by createdBy",
description = "Returns a list of syncStates that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SyncStateResponse>> findByCreatedBy(
	...
	var future = applicationService.findBySyncStateCreatedBy(SyncStateCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find syncState by createdBy: {}", e.getMessage(), e);
