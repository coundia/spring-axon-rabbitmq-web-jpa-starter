...
@Tag(name = "SyncState Queries", description = "Endpoints for querying syncStates by syncAt")
@Slf4j
public class FindBySyncAtSyncStateController {

private final SyncStateReadApplicationService applicationService;

public FindBySyncAtSyncStateController(SyncStateReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/syncAt")
@Operation(
summary = "Find syncState by syncAt",
description = "Returns a list of syncStates that match the given syncAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<SyncStateResponse>> findBySyncAt(
	...
	var future = applicationService.findBySyncStateSyncAt(SyncStateSyncAt
	.create(syncAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find syncState by syncAt: {}", e.getMessage(), e);
