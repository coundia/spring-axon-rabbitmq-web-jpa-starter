...
@Tag(name = "ChangeLog Queries", description = "Endpoints for querying changeLogs by status")
@Slf4j
public class FindByStatusChangeLogController {

private final ChangeLogReadApplicationService applicationService;

public FindByStatusChangeLogController(ChangeLogReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/status")
@Operation(
summary = "Find changeLog by status",
description = "Returns a list of changeLogs that match the given status"
)
@ApiResponses(value = {
...
public ResponseEntity<List<ChangeLogResponse>> findByStatus(
	...
	var future = applicationService.findByChangeLogStatus(ChangeLogStatus
	.create(status) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find changeLog by status: {}", e.getMessage(), e);
