...
public class DebtGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Debt> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Debt {} not found", objectId);
			return false;
		}
		Debt tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Debt {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Debt {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Debt {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Debt> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Debt {} not found", objectId);
			return false;
		}
		Debt entity = opt.get();
