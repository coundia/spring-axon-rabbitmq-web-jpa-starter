...
@Tag(name = "Debt Queries", description = "Endpoints for querying debts by remoteId")
@Slf4j
public class FindByRemoteIdDebtController {

private final DebtReadApplicationService applicationService;

public FindByRemoteIdDebtController(DebtReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/remoteId")
@Operation(
summary = "Find debt by remoteId",
description = "Returns a list of debts that match the given remoteId"
)
@ApiResponses(value = {
...
public ResponseEntity<List<DebtResponse>> findByRemoteId(
	...
	var future = applicationService.findByDebtRemoteId(DebtRemoteId
	.create(remoteId) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find debt by remoteId: {}", e.getMessage(), e);
