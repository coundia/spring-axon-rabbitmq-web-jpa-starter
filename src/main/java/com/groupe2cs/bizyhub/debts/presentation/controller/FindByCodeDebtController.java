...
@Tag(name = "Debt Queries", description = "Endpoints for querying debts by code")
@Slf4j
public class FindByCodeDebtController {

private final DebtReadApplicationService applicationService;

public FindByCodeDebtController(DebtReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/code")
@Operation(
summary = "Find debt by code",
description = "Returns a list of debts that match the given code"
)
@ApiResponses(value = {
...
public ResponseEntity<List<DebtResponse>> findByCode(
	...
	var future = applicationService.findByDebtCode(DebtCode
	.create(code) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find debt by code: {}", e.getMessage(), e);
