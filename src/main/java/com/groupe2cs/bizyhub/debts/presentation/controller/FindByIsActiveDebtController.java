...
@Tag(name = "Debt Queries", description = "Endpoints for querying debts by isActive")
@Slf4j
public class FindByIsActiveDebtController {

private final DebtReadApplicationService applicationService;

public FindByIsActiveDebtController(DebtReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/isActive")
@Operation(
summary = "Find debt by isActive",
description = "Returns a list of debts that match the given isActive"
)
@ApiResponses(value = {
...
public ResponseEntity<List<DebtResponse>> findByIsActive(
	...
	var future = applicationService.findByDebtIsActive(DebtIsActive
	.create(isActive) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find debt by isActive: {}", e.getMessage(), e);
