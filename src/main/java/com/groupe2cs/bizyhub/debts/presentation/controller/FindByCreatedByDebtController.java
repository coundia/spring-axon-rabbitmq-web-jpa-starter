...
@Tag(name = "Debt Queries", description = "Endpoints for querying debts by createdBy")
@Slf4j
public class FindByCreatedByDebtController {

private final DebtReadApplicationService applicationService;

public FindByCreatedByDebtController(DebtReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find debt by createdBy",
description = "Returns a list of debts that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<DebtResponse>> findByCreatedBy(
	...
	var future = applicationService.findByDebtCreatedBy(DebtCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find debt by createdBy: {}", e.getMessage(), e);
