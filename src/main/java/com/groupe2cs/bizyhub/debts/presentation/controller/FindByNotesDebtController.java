...
@Tag(name = "Debt Queries", description = "Endpoints for querying debts by notes")
@Slf4j
public class FindByNotesDebtController {

private final DebtReadApplicationService applicationService;

public FindByNotesDebtController(DebtReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/notes")
@Operation(
summary = "Find debt by notes",
description = "Returns a list of debts that match the given notes"
)
@ApiResponses(value = {
...
public ResponseEntity<List<DebtResponse>> findByNotes(
	...
	var future = applicationService.findByDebtNotes(DebtNotes
	.create(notes) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find debt by notes: {}", e.getMessage(), e);
