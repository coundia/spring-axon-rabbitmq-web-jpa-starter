...
@Tag(name = "Debt Queries", description = "Endpoints for querying debts by tenant")
@Slf4j
public class FindByTenantDebtController {

private final DebtReadApplicationService applicationService;

public FindByTenantDebtController(DebtReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find debt by tenant",
description = "Returns a list of debts that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<DebtResponse>> findByTenant(
	...
	var future = applicationService.findByDebtTenant(DebtTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find debt by tenant: {}", e.getMessage(), e);
