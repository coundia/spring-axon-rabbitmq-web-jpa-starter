...
public class ApiKeyGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<ApiKey> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: ApiKey {} not found", objectId);
			return false;
		}
		ApiKey tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for ApiKey {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on ApiKey {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<ApiKey> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: ApiKey {} not found", objectId);
			return false;
		}
