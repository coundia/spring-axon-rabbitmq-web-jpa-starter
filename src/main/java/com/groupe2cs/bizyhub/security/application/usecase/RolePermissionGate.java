...
public class RolePermissionGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<RolePermission> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: RolePermission {} not found", objectId);
			return false;
		}
		RolePermission tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for RolePermission {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on RolePermission {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on RolePermission {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<RolePermission> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: RolePermission {} not found", objectId);
			return false;
		}
		RolePermission entity = opt.get();


		boolean result = entity.getCreatedBy() != null && userId.equals(entity.getCreatedBy().getId());
		log.info("canDelete result for userId {} on RolePermission {}: {}", userId, objectId, result);
		return result;
	}
