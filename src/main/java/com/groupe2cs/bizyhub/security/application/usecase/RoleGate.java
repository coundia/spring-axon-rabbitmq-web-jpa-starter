...
public class RoleGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Role> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Role {} not found", objectId);
			return false;
		}
		Role tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Role {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Role {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Role {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Role> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Role {} not found", objectId);
			return false;
		}
		Role entity = opt.get();
