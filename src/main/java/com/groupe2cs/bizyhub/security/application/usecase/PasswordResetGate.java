...
public class PasswordResetGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<PasswordReset> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: PasswordReset {} not found", objectId);
			return false;
		}
		PasswordReset tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for PasswordReset {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on PasswordReset {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on PasswordReset {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<PasswordReset> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: PasswordReset {} not found", objectId);
			return false;
		}
