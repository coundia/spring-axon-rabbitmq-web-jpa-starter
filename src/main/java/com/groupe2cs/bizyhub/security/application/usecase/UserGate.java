...
Optional<CustomUser> opt = repository.findById(objectId);
	if (opt.isEmpty()) {
	log.warn("canRead denied: transaction {} not found", objectId);
	return false;
	}
	CustomUser tx = opt.get();
	...
	log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for CustomUser {}", currentUserId, owner, sameTenant, result, objectId);
	return result;
	}
	...
	log.info("canEdit result for user {} on CustomUser {}: {}", auth != null ? auth.getName() : null, objectId, result);
	return result;
	}
	...
	boolean result = canRead(auth, objectId);
	log.info("canDelete result for user {} on CustomUser {}: {}", auth != null ? auth.getName() : null, objectId, result);
	return result;
	}
