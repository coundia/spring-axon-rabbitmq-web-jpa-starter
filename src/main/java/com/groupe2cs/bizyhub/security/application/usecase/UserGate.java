...
public class UserGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<User> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: User {} not found", objectId);
			return false;
		}
		User tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for User {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on User {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on User {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<User> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: User {} not found", objectId);
			return false;
		}
		User entity = opt.get();
