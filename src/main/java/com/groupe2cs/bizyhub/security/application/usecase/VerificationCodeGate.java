...
public class VerificationCodeGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<VerificationCode> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: VerificationCode {} not found", objectId);
			return false;
		}
		VerificationCode tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for VerificationCode {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on VerificationCode {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on VerificationCode {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<VerificationCode> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: VerificationCode {} not found", objectId);
			return false;
		}
