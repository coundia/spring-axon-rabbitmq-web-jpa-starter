...
public class UpdatePermissionController {
	...
	public ResponseEntity<PermissionResponse> updatePermission(@Valid @PathVariable String id, @RequestBody PermissionRequest request) {
		{
			try {

				PermissionResponse response = applicationService.updatePermission(PermissionId.create(id), request);
