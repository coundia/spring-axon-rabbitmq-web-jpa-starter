...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by name")
@Slf4j
public class FindByNameCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByNameCompanyController(CompanyReadApplicationService  applicationService) {
...
description = "Returns a list of companys that match the given name"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByName(
	...
	var future = applicationService.findByCompanyName(CompanyName
	.create(name) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by name: {}", e.getMessage(), e);
