...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by addressLine2")
@Slf4j
public class FindByAddressLine2CompanyController {

private final CompanyReadApplicationService applicationService;

public FindByAddressLine2CompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/addressLine2")
@Operation(
summary = "Find company by addressLine2",
description = "Returns a list of companys that match the given addressLine2"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByAddressLine2(
	...
	var future = applicationService.findByCompanyAddressLine2(CompanyAddressLine2
	.create(addressLine2) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by addressLine2: {}", e.getMessage(), e);
