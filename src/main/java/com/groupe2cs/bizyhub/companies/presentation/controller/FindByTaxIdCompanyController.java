...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by taxId")
@Slf4j
public class FindByTaxIdCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByTaxIdCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/taxId")
@Operation(
summary = "Find company by taxId",
description = "Returns a list of companys that match the given taxId"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByTaxId(
	@AuthenticationPrincipal Jwt jwt,
	@Parameter(description = "Value of the taxId to filter by", required = true)
	...
	var future = applicationService.findByCompanyTaxId(CompanyTaxId
	.create(taxId) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by taxId: {}", e.getMessage(), e);
