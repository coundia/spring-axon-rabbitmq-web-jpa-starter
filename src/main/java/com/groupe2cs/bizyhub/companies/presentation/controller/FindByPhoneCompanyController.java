...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by phone")
@Slf4j
public class FindByPhoneCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByPhoneCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/phone")
@Operation(
summary = "Find company by phone",
description = "Returns a list of companys that match the given phone"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByPhone(
	...
	var future = applicationService.findByCompanyPhone(CompanyPhone
	.create(phone) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by phone: {}", e.getMessage(), e);
