...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by remoteId")
@Slf4j
public class FindByRemoteIdCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByRemoteIdCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/remoteId")
@Operation(
summary = "Find company by remoteId",
description = "Returns a list of companys that match the given remoteId"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByRemoteId(
	...
	var future = applicationService.findByCompanyRemoteId(CompanyRemoteId
	.create(remoteId) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by remoteId: {}", e.getMessage(), e);
