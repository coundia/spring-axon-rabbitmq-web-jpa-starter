...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by city")
@Slf4j
public class FindByCityCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByCityCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/city")
@Operation(
summary = "Find company by city",
description = "Returns a list of companys that match the given city"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByCity(
	...
	var future = applicationService.findByCompanyCity(CompanyCity
	.create(city) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by city: {}", e.getMessage(), e);
