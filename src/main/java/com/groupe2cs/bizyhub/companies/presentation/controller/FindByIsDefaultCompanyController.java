...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by isDefault")
@Slf4j
public class FindByIsDefaultCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByIsDefaultCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/isDefault")
@Operation(
summary = "Find company by isDefault",
description = "Returns a list of companys that match the given isDefault"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByIsDefault(
	...
	var future = applicationService.findByCompanyIsDefault(CompanyIsDefault
	.create(isDefault) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by isDefault: {}", e.getMessage(), e);
