...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by email")
@Slf4j
public class FindByEmailCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByEmailCompanyController(CompanyReadApplicationService  applicationService) {
...
description = "Returns a list of companys that match the given email"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByEmail(
	...
	var future = applicationService.findByCompanyEmail(CompanyEmail
	.create(email) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by email: {}", e.getMessage(), e);
