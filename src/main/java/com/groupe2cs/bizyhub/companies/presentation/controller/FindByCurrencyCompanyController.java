...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by currency")
@Slf4j
public class FindByCurrencyCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByCurrencyCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/currency")
@Operation(
summary = "Find company by currency",
description = "Returns a list of companys that match the given currency"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByCurrency(
	...
	var future = applicationService.findByCompanyCurrency(CompanyCurrency
	.create(currency) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by currency: {}", e.getMessage(), e);
