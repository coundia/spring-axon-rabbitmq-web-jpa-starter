...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by createdBy")
@Slf4j
public class FindByCreatedByCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByCreatedByCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find company by createdBy",
description = "Returns a list of companys that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByCreatedBy(
	...
	var future = applicationService.findByCompanyCreatedBy(CompanyCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by createdBy: {}", e.getMessage(), e);
