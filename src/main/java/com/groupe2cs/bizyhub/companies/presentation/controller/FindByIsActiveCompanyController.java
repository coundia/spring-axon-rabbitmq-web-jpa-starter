...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by isActive")
@Slf4j
public class FindByIsActiveCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByIsActiveCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/isActive")
@Operation(
summary = "Find company by isActive",
description = "Returns a list of companys that match the given isActive"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByIsActive(
	...
	var future = applicationService.findByCompanyIsActive(CompanyIsActive
	.create(isActive) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by isActive: {}", e.getMessage(), e);
