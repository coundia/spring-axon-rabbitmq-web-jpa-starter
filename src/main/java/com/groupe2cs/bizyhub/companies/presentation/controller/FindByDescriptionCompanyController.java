...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by description")
@Slf4j
public class FindByDescriptionCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByDescriptionCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/description")
@Operation(
summary = "Find company by description",
description = "Returns a list of companys that match the given description"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByDescription(
	...
	var future = applicationService.findByCompanyDescription(CompanyDescription
	.create(description) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by description: {}", e.getMessage(), e);
