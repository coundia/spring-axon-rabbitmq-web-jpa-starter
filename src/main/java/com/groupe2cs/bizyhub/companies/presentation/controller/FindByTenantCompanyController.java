...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by tenant")
@Slf4j
public class FindByTenantCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByTenantCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find company by tenant",
description = "Returns a list of companys that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByTenant(
	...
	var future = applicationService.findByCompanyTenant(CompanyTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by tenant: {}", e.getMessage(), e);
