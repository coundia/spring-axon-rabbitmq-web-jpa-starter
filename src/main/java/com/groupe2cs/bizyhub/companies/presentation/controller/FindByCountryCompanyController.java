...
@Tag(name = "Company Queries", description = "Endpoints for querying companys by country")
@Slf4j
public class FindByCountryCompanyController {

private final CompanyReadApplicationService applicationService;

public FindByCountryCompanyController(CompanyReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/country")
@Operation(
summary = "Find company by country",
description = "Returns a list of companys that match the given country"
)
@ApiResponses(value = {
...
public ResponseEntity<List<CompanyResponse>> findByCountry(
	...
	var future = applicationService.findByCompanyCountry(CompanyCountry
	.create(country) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find company by country: {}", e.getMessage(), e);
