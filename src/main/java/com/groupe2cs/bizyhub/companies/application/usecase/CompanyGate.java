...
public class CompanyGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Company> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Company {} not found", objectId);
			return false;
		}
		Company tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Company {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Company {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Company {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Company> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Company {} not found", objectId);
			return false;
		}
