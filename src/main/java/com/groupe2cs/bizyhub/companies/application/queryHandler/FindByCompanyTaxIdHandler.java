...
import com.groupe2cs.bizyhub.companies.application.query.FindByCompanyTaxIdQuery;
...
public class FindByCompanyTaxIdHandler {

	private final CompanyRepository repository;

	@QueryHandler
	public List<CompanyResponse> handle(FindByCompanyTaxIdQuery query) {

		MetaRequest metaRequest = query.getMetaRequest();
		List<Company> entities = null;
		String value = query.getTaxId().value();

		if (metaRequest.isAdmin()) {
			entities = repository.findByTaxIdAndTenantId(value, metaRequest.getTenantId());
		} else {
			entities = repository.findByTaxIdAndCreatedById(value, metaRequest.getUserId());
		}
