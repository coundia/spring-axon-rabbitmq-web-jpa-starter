...
public class CompanyReadApplicationService {
	...
	public List<CompanyResponse> findByCompanyStatus(
			CompanyStatus value,
			MetaRequest metaRequest
	) {

		FindByCompanyStatusQuery query = new FindByCompanyStatusQuery(value, metaRequest);
		CompletableFuture<List<CompanyResponse>> future = queryGateway.query(query,
				org.axonframework.messaging.responsetypes.ResponseTypes.multipleInstancesOf(CompanyResponse.class));
		return future.join();
	}

	public List<CompanyResponse> findByCompanyIsPublic(
			CompanyIsPublic value,
			MetaRequest metaRequest
	) {

		FindByCompanyIsPublicQuery query = new FindByCompanyIsPublicQuery(value, metaRequest);
		CompletableFuture<List<CompanyResponse>> future = queryGateway.query(query,
				org.axonframework.messaging.responsetypes.ResponseTypes.multipleInstancesOf(CompanyResponse.class));
		return future.join();
	}
