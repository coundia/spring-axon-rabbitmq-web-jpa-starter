...
import com.groupe2cs.bizyhub.companies.application.query.FindByCompanyIsDefaultQuery;
...
public class FindByCompanyIsDefaultHandler {

	private final CompanyRepository repository;

	@QueryHandler
	public List<CompanyResponse> handle(FindByCompanyIsDefaultQuery query) {
		...
		if (metaRequest.isAdmin()) {
			entities = repository.findByIsDefaultAndTenantId(value, metaRequest.getTenantId());
		} else {
			entities = repository.findByIsDefaultAndCreatedById(value, metaRequest.getUserId());
		}
