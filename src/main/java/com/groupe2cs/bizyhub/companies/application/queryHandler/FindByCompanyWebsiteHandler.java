...
import com.groupe2cs.bizyhub.companies.application.query.FindByCompanyWebsiteQuery;
...
public class FindByCompanyWebsiteHandler {

	private final CompanyRepository repository;

	@QueryHandler
	public List<CompanyResponse> handle(FindByCompanyWebsiteQuery query) {

		MetaRequest metaRequest = query.getMetaRequest();
		List<Company> entities = null;
		String value = query.getWebsite().value();

		if (metaRequest.isAdmin()) {
			entities = repository.findByWebsiteAndTenantId(value, metaRequest.getTenantId());
		} else {
			entities = repository.findByWebsiteAndCreatedById(value, metaRequest.getUserId());
		}
