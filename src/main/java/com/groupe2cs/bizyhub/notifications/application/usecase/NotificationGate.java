...
public class NotificationGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<Notification> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: Notification {} not found", objectId);
			return false;
		}
		Notification tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for Notification {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on Notification {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on Notification {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<Notification> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: Notification {} not found", objectId);
			return false;
		}
