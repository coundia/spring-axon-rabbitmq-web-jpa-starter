...
import com.groupe2cs.bizyhub.notifications.application.query.FindByNotificationDeviceTokenQuery;
...
public class FindByNotificationDeviceTokenHandler {

	private final NotificationRepository repository;

	@QueryHandler
	public List<NotificationResponse> handle(FindByNotificationDeviceTokenQuery query) {

		MetaRequest metaRequest = query.getMetaRequest();
		List<Notification> entities = null;
		String value = query.getDeviceToken().value();

		if (metaRequest.isAdmin()) {
			entities = repository.findByDeviceTokenAndTenantId(value, metaRequest.getTenantId());
		} else {
			entities = repository.findByDeviceTokenAndCreatedById(value, metaRequest.getUserId());
		}
