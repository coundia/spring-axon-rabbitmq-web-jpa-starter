...
@Tag(name = "Notification Queries", description = "Endpoints for querying notifications by reference")
@Slf4j
public class FindByReferenceNotificationController {

private final NotificationReadApplicationService applicationService;

public FindByReferenceNotificationController(NotificationReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/reference")
@Operation(
summary = "Find notification by reference",
description = "Returns a list of notifications that match the given reference"
)
@ApiResponses(value = {
...
public ResponseEntity<List<NotificationResponse>> findByReference(
	...
	var future = applicationService.findByNotificationReference(NotificationReference
	.create(reference) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find notification by reference: {}", e.getMessage(), e);
