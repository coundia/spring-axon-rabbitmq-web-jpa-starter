...
@Tag(name = "Notification Queries", description = "Endpoints for querying notifications by status")
@Slf4j
public class FindByStatusNotificationController {

private final NotificationReadApplicationService applicationService;

public FindByStatusNotificationController(NotificationReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/status")
@Operation(
summary = "Find notification by status",
description = "Returns a list of notifications that match the given status"
)
@ApiResponses(value = {
...
public ResponseEntity<List<NotificationResponse>> findByStatus(
	...
	var future = applicationService.findByNotificationStatus(NotificationStatus
	.create(status) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find notification by status: {}", e.getMessage(), e);
