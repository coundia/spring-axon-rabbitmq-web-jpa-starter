...
@Tag(name = "Notification Queries", description = "Endpoints for querying notifications by updatedAt")
@Slf4j
public class FindByUpdatedAtNotificationController {

private final NotificationReadApplicationService applicationService;

public FindByUpdatedAtNotificationController(NotificationReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/updatedAt")
@Operation(
summary = "Find notification by updatedAt",
description = "Returns a list of notifications that match the given updatedAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<NotificationResponse>> findByUpdatedAt(
	...
	var future = applicationService.findByNotificationUpdatedAt(NotificationUpdatedAt
	.create(updatedAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find notification by updatedAt: {}", e.getMessage(), e);
