...
@Tag(name = "Account Queries", description = "Endpoints for querying accounts by syncAt")
@Slf4j
public class FindBySyncAtAccountController {

private final AccountReadApplicationService applicationService;

public FindBySyncAtAccountController(AccountReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/syncAt")
@Operation(
summary = "Find account by syncAt",
description = "Returns a list of accounts that match the given syncAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountResponse>> findBySyncAt(
	...
	var future = applicationService.findByAccountSyncAt(AccountSyncAt
	.create(syncAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find account by syncAt: {}", e.getMessage(), e);
