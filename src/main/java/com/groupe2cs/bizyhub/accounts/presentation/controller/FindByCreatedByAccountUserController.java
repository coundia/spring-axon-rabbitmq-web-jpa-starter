...
@Tag(name = "AccountUser Queries", description = "Endpoints for querying accountUsers by createdBy")
@Slf4j
public class FindByCreatedByAccountUserController {

private final AccountUserReadApplicationService applicationService;

public FindByCreatedByAccountUserController(AccountUserReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find accountUser by createdBy",
description = "Returns a list of accountUsers that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountUserResponse>> findByCreatedBy(
	...
	var future = applicationService.findByAccountUserCreatedBy(AccountUserCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find accountUser by createdBy: {}", e.getMessage(), e);
