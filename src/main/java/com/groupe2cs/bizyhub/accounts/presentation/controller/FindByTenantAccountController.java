...
@Tag(name = "Account Queries", description = "Endpoints for querying accounts by tenant")
@Slf4j
public class FindByTenantAccountController {

private final AccountReadApplicationService applicationService;

public FindByTenantAccountController(AccountReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find account by tenant",
description = "Returns a list of accounts that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountResponse>> findByTenant(
	...
	var future = applicationService.findByAccountTenant(AccountTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find account by tenant: {}", e.getMessage(), e);
