...
@Tag(name = "Account Queries", description = "Endpoints for querying accounts by balance")
@Slf4j
public class FindByBalanceAccountController {

private final AccountReadApplicationService applicationService;

public FindByBalanceAccountController(AccountReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/balance")
@Operation(
summary = "Find account by balance",
description = "Returns a list of accounts that match the given balance"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountResponse>> findByBalance(
	...
	var future = applicationService.findByAccountBalance(AccountBalance
	.create(balance) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find account by balance: {}", e.getMessage(), e);
