...
@Tag(name = "AccountUser Queries", description = "Endpoints for querying accountUsers by syncAt")
@Slf4j
public class FindBySyncAtAccountUserController {

private final AccountUserReadApplicationService applicationService;

public FindBySyncAtAccountUserController(AccountUserReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/syncAt")
@Operation(
summary = "Find accountUser by syncAt",
description = "Returns a list of accountUsers that match the given syncAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountUserResponse>> findBySyncAt(
	...
	var future = applicationService.findByAccountUserSyncAt(AccountUserSyncAt
	.create(syncAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find accountUser by syncAt: {}", e.getMessage(), e);
