...
@Tag(name = "Account Queries", description = "Endpoints for querying accounts by description")
@Slf4j
public class FindByDescriptionAccountController {

private final AccountReadApplicationService applicationService;

public FindByDescriptionAccountController(AccountReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/description")
@Operation(
summary = "Find account by description",
description = "Returns a list of accounts that match the given description"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountResponse>> findByDescription(
	...
	var future = applicationService.findByAccountDescription(AccountDescription
	.create(description) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find account by description: {}", e.getMessage(), e);
