...
@Tag(name = "AccountUser Queries", description = "Endpoints for querying accountUsers by details")
@Slf4j
public class FindByDetailsAccountUserController {

private final AccountUserReadApplicationService applicationService;

public FindByDetailsAccountUserController(AccountUserReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/details")
@Operation(
summary = "Find accountUser by details",
description = "Returns a list of accountUsers that match the given details"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountUserResponse>> findByDetails(
	...
	var future = applicationService.findByAccountUserDetails(AccountUserDetails
	.create(details) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find accountUser by details: {}", e.getMessage(), e);
