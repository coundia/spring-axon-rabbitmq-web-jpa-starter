...
@Tag(name = "Account Queries", description = "Endpoints for querying accounts by status")
@Slf4j
public class FindByStatusAccountController {

private final AccountReadApplicationService applicationService;

public FindByStatusAccountController(AccountReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/status")
@Operation(
summary = "Find account by status",
description = "Returns a list of accounts that match the given status"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountResponse>> findByStatus(
	...
	var future = applicationService.findByAccountStatus(AccountStatus
	.create(status) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find account by status: {}", e.getMessage(), e);
