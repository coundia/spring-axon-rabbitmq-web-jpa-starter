...
@Tag(name = "Account Queries", description = "Endpoints for querying accounts by code")
@Slf4j
public class FindByCodeAccountController {

private final AccountReadApplicationService applicationService;

public FindByCodeAccountController(AccountReadApplicationService  applicationService) {
...
description = "Returns a list of accounts that match the given code"
)
@ApiResponses(value = {
...
public ResponseEntity<List<AccountResponse>> findByCode(
	...
	var future = applicationService.findByAccountCode(AccountCode
	.create(code) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find account by code: {}", e.getMessage(), e);
