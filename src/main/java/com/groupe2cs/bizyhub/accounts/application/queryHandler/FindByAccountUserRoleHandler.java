...
import com.groupe2cs.bizyhub.accounts.application.query.FindByAccountUserRoleQuery;
...
public class FindByAccountUserRoleHandler {

	private final AccountUserRepository repository;

	@QueryHandler
	public List<AccountUserResponse> handle(FindByAccountUserRoleQuery query) {
		...
		if (metaRequest.isAdmin()) {
			entities = repository.findByRoleAndTenantId(value, metaRequest.getTenantId());
		} else {
			entities = repository.findByRoleAndCreatedById(value, metaRequest.getUserId());
		}
