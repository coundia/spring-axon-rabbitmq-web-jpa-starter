...
public class StockMovementGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<StockMovement> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: StockMovement {} not found", objectId);
			return false;
		}
		StockMovement tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for StockMovement {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on StockMovement {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on StockMovement {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<StockMovement> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: StockMovement {} not found", objectId);
			return false;
		}
