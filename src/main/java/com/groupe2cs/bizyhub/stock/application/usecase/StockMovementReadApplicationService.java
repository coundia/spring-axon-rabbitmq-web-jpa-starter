...
public class StockMovementReadApplicationService {
	...
	public List<StockMovementResponse> findByStockMovementTypeStockMovement(
		...
		FindByStockMovementTypeStockMovementQuery
				query =
				new FindByStockMovementTypeStockMovementQuery(value, metaRequest);
	...
	public List<StockMovementResponse> findByStockMovementAccount(
			StockMovementAccount value,
			MetaRequest metaRequest
	) {

		FindByStockMovementAccountQuery query = new FindByStockMovementAccountQuery(value, metaRequest);
		CompletableFuture<List<StockMovementResponse>> future = queryGateway.query(query,
				org.axonframework.messaging.responsetypes.ResponseTypes.multipleInstancesOf(StockMovementResponse.class));
		return future.join();
	}
