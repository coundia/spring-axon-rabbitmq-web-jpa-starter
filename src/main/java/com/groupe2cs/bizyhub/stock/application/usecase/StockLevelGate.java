...
public class StockLevelGate {
	...
	public boolean canRead(Authentication auth, String objectId) {
		...
		Optional<StockLevel> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canRead denied: StockLevel {} not found", objectId);
			return false;
		}
		StockLevel tx = opt.get();
		...
		log.info("canRead result for user {}: owner={}, sameTenant={}, final={} for StockLevel {}",
	...
	public boolean canEdit(Authentication auth, String objectId) {
		...
		log.info("canEdit result for user {} on StockLevel {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(Authentication auth, String objectId) {
		...
		boolean result = canRead(auth, objectId);
		log.info("canDelete result for user {} on StockLevel {}: {}", auth != null ? auth.getName() : null, objectId, result);
		return result;
	}
	...
	public boolean canDelete(String userId, String objectId) {
		...
		Optional<StockLevel> opt = repository.findById(objectId);
		if (opt.isEmpty()) {
			log.warn("canDelete denied: StockLevel {} not found", objectId);
			return false;
		}
