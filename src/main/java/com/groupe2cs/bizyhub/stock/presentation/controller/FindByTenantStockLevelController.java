...
@Tag(name = "StockLevel Queries", description = "Endpoints for querying stockLevels by tenant")
@Slf4j
public class FindByTenantStockLevelController {

private final StockLevelReadApplicationService applicationService;

public FindByTenantStockLevelController(StockLevelReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/tenant")
@Operation(
summary = "Find stockLevel by tenant",
description = "Returns a list of stockLevels that match the given tenant"
)
@ApiResponses(value = {
...
public ResponseEntity<List<StockLevelResponse>> findByTenant(
	...
	var future = applicationService.findByStockLevelTenant(StockLevelTenant
	.create(tenant) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find stockLevel by tenant: {}", e.getMessage(), e);
