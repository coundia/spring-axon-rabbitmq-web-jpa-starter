...
@Tag(name = "StockMovement Queries", description = "Endpoints for querying stockMovements by createdBy")
@Slf4j
public class FindByCreatedByStockMovementController {

private final StockMovementReadApplicationService applicationService;

public FindByCreatedByStockMovementController(StockMovementReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find stockMovement by createdBy",
description = "Returns a list of stockMovements that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<StockMovementResponse>> findByCreatedBy(
	...
	var future = applicationService.findByStockMovementCreatedBy(StockMovementCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find stockMovement by createdBy: {}", e.getMessage(), e);
