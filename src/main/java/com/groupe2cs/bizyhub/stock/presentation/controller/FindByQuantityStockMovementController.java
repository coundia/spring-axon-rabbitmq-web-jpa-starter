...
@Tag(name = "StockMovement Queries", description = "Endpoints for querying stockMovements by quantity")
@Slf4j
public class FindByQuantityStockMovementController {

private final StockMovementReadApplicationService applicationService;

public FindByQuantityStockMovementController(StockMovementReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/quantity")
@Operation(
summary = "Find stockMovement by quantity",
description = "Returns a list of stockMovements that match the given quantity"
)
@ApiResponses(value = {
...
public ResponseEntity<List<StockMovementResponse>> findByQuantity(
	...
	var future = applicationService.findByStockMovementQuantity(StockMovementQuantity
	.create(quantity) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find stockMovement by quantity: {}", e.getMessage(), e);
