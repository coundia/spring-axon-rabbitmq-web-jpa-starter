...
@Tag(name = "StockLevel Queries", description = "Endpoints for querying stockLevels by createdBy")
@Slf4j
public class FindByCreatedByStockLevelController {

private final StockLevelReadApplicationService applicationService;

public FindByCreatedByStockLevelController(StockLevelReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/createdBy")
@Operation(
summary = "Find stockLevel by createdBy",
description = "Returns a list of stockLevels that match the given createdBy"
)
@ApiResponses(value = {
...
public ResponseEntity<List<StockLevelResponse>> findByCreatedBy(
	...
	var future = applicationService.findByStockLevelCreatedBy(StockLevelCreatedBy
	.create(createdBy) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find stockLevel by createdBy: {}", e.getMessage(), e);
