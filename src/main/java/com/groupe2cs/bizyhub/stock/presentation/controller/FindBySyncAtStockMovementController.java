...
@Tag(name = "StockMovement Queries", description = "Endpoints for querying stockMovements by syncAt")
@Slf4j
public class FindBySyncAtStockMovementController {

private final StockMovementReadApplicationService applicationService;

public FindBySyncAtStockMovementController(StockMovementReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/syncAt")
@Operation(
summary = "Find stockMovement by syncAt",
description = "Returns a list of stockMovements that match the given syncAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<StockMovementResponse>> findBySyncAt(
	...
	var future = applicationService.findByStockMovementSyncAt(StockMovementSyncAt
	.create(syncAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find stockMovement by syncAt: {}", e.getMessage(), e);
