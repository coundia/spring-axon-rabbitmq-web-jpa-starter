...
@Tag(name = "Balance Queries", description = "Endpoints for querying balances by previousBalance")
@Slf4j
public class FindByPreviousBalanceBalanceController {

private final BalanceReadApplicationService applicationService;

public FindByPreviousBalanceBalanceController(BalanceReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/previousBalance")
@Operation(
summary = "Find balance by previousBalance",
description = "Returns a list of balances that match the given previousBalance"
)
@ApiResponses(value = {
...
public ResponseEntity<List<BalanceResponse>> findByPreviousBalance(
	...
	var future = applicationService.findByBalancePreviousBalance(BalancePreviousBalance
	.create(previousBalance) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find balance by previousBalance: {}", e.getMessage(), e);
