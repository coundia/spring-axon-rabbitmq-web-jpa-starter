...
@Tag(name = "Balance Queries", description = "Endpoints for querying balances by updatedAt")
@Slf4j
public class FindByUpdatedAtBalanceController {

private final BalanceReadApplicationService applicationService;

public FindByUpdatedAtBalanceController(BalanceReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/updatedAt")
@Operation(
summary = "Find balance by updatedAt",
description = "Returns a list of balances that match the given updatedAt"
)
@ApiResponses(value = {
...
public ResponseEntity<List<BalanceResponse>> findByUpdatedAt(
	...
	var future = applicationService.findByBalanceUpdatedAt(BalanceUpdatedAt
	.create(updatedAt) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find balance by updatedAt: {}", e.getMessage(), e);
