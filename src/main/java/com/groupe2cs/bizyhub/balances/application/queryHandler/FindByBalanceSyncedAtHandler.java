...
public List<BalanceResponse> handle(FindByBalanceSyncedAtQuery query) {
	 ...
	 if(metaRequest.isAdmin()) {
	    entities = repository.findBySyncedAtAndTenantId(value, metaRequest.getTenantId());
	 }else{
	    entities = repository.findBySyncedAtAndCreatedById(value, metaRequest.getUserId());
	 }
