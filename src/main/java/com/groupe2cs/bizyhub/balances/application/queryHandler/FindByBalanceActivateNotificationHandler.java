...
public List<BalanceResponse> handle(FindByBalanceActivateNotificationQuery query) {
	 ...
	 if(metaRequest.isAdmin()) {
	    entities = repository.findByActivateNotificationAndTenantId(value, metaRequest.getTenantId());
	 }else{
	    entities = repository.findByActivateNotificationAndCreatedById(value, metaRequest.getUserId());
	 }
