...
public List<BalanceResponse> handle(FindByBalanceIsAllowCreditQuery query) {
	 ...
	 if(metaRequest.isAdmin()) {
	    entities = repository.findByIsAllowCreditAndTenantId(value, metaRequest.getTenantId());
	 }else{
	    entities = repository.findByIsAllowCreditAndCreatedById(value, metaRequest.getUserId());
	 }
