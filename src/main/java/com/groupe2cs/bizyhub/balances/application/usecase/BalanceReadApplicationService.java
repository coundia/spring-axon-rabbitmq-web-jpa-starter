...
public List<BalanceResponse> findByBalanceUpdatedAt(
	BalanceUpdatedAt value,
	MetaRequest metaRequest
	) {

	FindByBalanceUpdatedAtQuery query = new FindByBalanceUpdatedAtQuery(value,metaRequest);
	CompletableFuture<List<BalanceResponse>> future = queryGateway.query(query,
	org.axonframework.messaging.responsetypes.ResponseTypes.multipleInstancesOf(BalanceResponse.class));
	return future.join();
}
public List<BalanceResponse> findByBalanceReference(
	BalanceReference value,
	MetaRequest metaRequest
	) {

	FindByBalanceReferenceQuery query = new FindByBalanceReferenceQuery(value,metaRequest);
	CompletableFuture<List<BalanceResponse>> future = queryGateway.query(query,
	org.axonframework.messaging.responsetypes.ResponseTypes.multipleInstancesOf(BalanceResponse.class));
	return future.join();
}
