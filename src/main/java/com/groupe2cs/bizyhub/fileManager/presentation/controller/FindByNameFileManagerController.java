...
@Tag(name = "FileManager Queries", description = "Endpoints for querying fileManagers by name")
@Slf4j
public class FindByNameFileManagerController {

private final FileManagerReadApplicationService applicationService;

public FindByNameFileManagerController(FileManagerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/name")
@Operation(
summary = "Find fileManager by name",
description = "Returns a list of fileManagers that match the given name"
)
@ApiResponses(value = {
...
public ResponseEntity<List<FileManagerResponse>> findByName(
	...
	var future = applicationService.findByFileManagerName(FileManagerName
	.create(name) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find fileManager by name: {}", e.getMessage(), e);
