...
@Tag(name = "FileManager Queries", description = "Endpoints for querying fileManagers by path")
@Slf4j
public class FindByPathFileManagerController {

private final FileManagerReadApplicationService applicationService;

public FindByPathFileManagerController(FileManagerReadApplicationService  applicationService) {
	this.applicationService = applicationService;
}

@GetMapping("/path")
@Operation(
summary = "Find fileManager by path",
description = "Returns a list of fileManagers that match the given path"
)
@ApiResponses(value = {
...
public ResponseEntity<List<FileManagerResponse>> findByPath(
	@AuthenticationPrincipal Jwt jwt,
	@Parameter(description = "Value of the path to filter by", required = true)
	...
	var future = applicationService.findByFileManagerPath(FileManagerPath
	.create(path) , metaRequest);

	return ResponseEntity.ok(future);
	} catch (Exception e) {
	log.error("Failed to find fileManager by path: {}", e.getMessage(), e);
